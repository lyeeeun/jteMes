...
<div id="grid-content" class="routing grid-content">
	<table class="routingMgtForm jteRoutingGrid">
		<colspan>
			<col class="tb-col-left-content">
			<col class="tb-col-right-content">
		</colspan>
		<tr>
			<td class="left-content-box" style="vertical-align:top;"><div id="jteSingleGrid"></div></td>
			<td class="right-content-box"><button id="routingAddBtn" class="k-button k-primary routingAddBtn" type='button' onclick="lfn_jtePop_open('NEW', '')" style="display:none;">
			<spring:message code = "btn_newCreat"></spring:message></button><div style="overflow:auto; width: 1255px;"><div id="routingTree"></div></div></td>		<!-- 신규생성 -->
		</tr>
	</table>	
</div>
...
<div style="display:none;">
	<div id="equipment_info">
		<div class="processInfoForm equipment_info_div">
			...
			<div class="equipment_info_right"><spring:message code = "pop_relEqmtList"></spring:message></div>			<!-- 관련 설비 목록 -->
...
this.lfn_init = function(){
	lfo_common.ctrlUrl ="/cform/basMgt/routing/routingMgt";
	//그리드 생성
	lfn_jteSgGrd_setGrd();
	//데이터 생성
		...
		success: function(data){
			...
			$.each(data.rows, function(index, item) {
				insertNode = {
						...
						itemId: item.itemId,
						colorScheme: "#d6e3ff", /*200416  우측  배경 색상 수정_LYM "#96a6ba" */
				};
...
function visualTemplate(options) {
	...
	g.append(new dataviz.diagram.TextBlock({
		...
		fontSize: 16,
		fontStyle: "bold",
		color: "#000000" /* 200416 n차 텍스트 색상 수정_LYM #ffffff */
	...
	g.append(new dataviz.diagram.TextBlock({
		...
		fontSize: 14,
		fontStyle: "bold",
		color: "#000000" /* 200416 RUT 텍스트 색상 수정_LYM #ffffff */
	}));
...
this.lfn_jtePop_open = function(mode, selectedItem){
	var options = {
			modal:true, 
			width: "700px", 
			height: "550px",  /** 200416 추가 생성에 따른 팝업 height 조정_LYM 535_JJW 535 -> 550**/
			id:"jtePopForm", 
	...
	if(mode=='NEW'){
		...
		options.height = "250px"; /** JJW 235 -> 250_LYM 235**/
	} else{
...
this.lfn_setBomTree = function(param) {
	...
	$.ajax({
		async:false,
		url : "/cform/basMgt/bom/bomMgt/getBomList",
		type: "POST",
...
this.lfn_equipmentInfo = function(mode) {
	var options = {
			modal:true, 
			width: "600px", 
			height: "480px", /* 200416 팝업창 높이 변경_LYM 500->480px*/
			id:"equipment_info", 
...
this.lfn_defectPopup = function(paramRoutingId) {
	...
	$.ajax({
		async:false,
		url : "/cform/basMgt/operMgt/toolMgt/getAllToolInfoList",
		type: "POST",
		data: "",
		success: function(data){
			$.each(data.result, function(index, item){
