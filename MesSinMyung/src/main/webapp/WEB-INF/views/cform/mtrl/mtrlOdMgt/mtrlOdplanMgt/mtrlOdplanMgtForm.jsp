	...
	<div id="jtePopForm">
		...
		<!-- 발주계획 -->
			...
			<colgroup> <!-- 13 27 13 27, 2200512 조회버튼 때문에 행 2개 추가-->
				...
				<!-- 배송비용 -->
					<td class="td-right-title"><i class="fas fa-caret-right"></i><spring:message code = "col_deliveryCost"></spring:message> : </td><!-- 배송비용 -->
					<td colspan="2"><input id="mtrlOrderDelivery" class="right-input" name="mtrlOrderDelivery" format="currency"/></td>
				</tr>
				<tr>
				<!-- 금액 -->
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_cost"></spring:message>
				<!-- 자재와 수량을 입력하면 기본적으로 자동계산됩니다. -->
					<i class="fas fa-info-circle" title="<spring:message code = "pop_mtrlQtyAuto"></spring:message>"></i> : </td>
					<td colspan="5"><input id="mtrlOrderCost" class="center-input" name="mtrlOrderCost" format="currency"/></td>
				</tr>
				<tr>
				<!-- 설명 -->
...
this.lfn_jteSgGrd_setGrd = function(){
	...
	*/
	...
	lfo_common.columns = [
		...
		{field: "mtrlOrderId", title:gfn_getMsg("col_mtrlItemCd"), width: "160px"},										//발주계획코드
		{field: "mtrlOrderState", template:"# var item = gfn_isNull(gfn_getCode(mtrlOrderState))== true ? '기초코드 없음' : gfn_getMsg('bc_'+ gfn_getCode(mtrlOrderState).cdId , gfn_getCode(mtrlOrderState).cdNm) # #= item #", 
			title:gfn_getMsg("col_orderSitDiv"), width: "80px;"},														//발주상황 구분
...
this.lfn_jtePop_open = function(mode, selectedItem){
	var options = {
		modal:true, 
		width: "700px", // 200512 JJW 그리드 더블 클릭 팝업 너비 700px
		height: "400px", // 200512 JJW 그리드 더블 클릭 팝업 높이 320px -> 380px
		id:lfo_common.popId, 
...
this.lfn_mtrlOrderForm_open = function(){
	var options = {
		...
		height: "600px", // 200519 JJw 발주주문서 팝업 높이 450px -> 600px
		id : "jteMtrlOrderPop", 
