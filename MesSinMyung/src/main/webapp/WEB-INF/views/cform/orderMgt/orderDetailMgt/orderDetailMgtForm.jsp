		...
		<form id="jtePopAddForm_form" class="jtePopAddForm_form orderDetailMgtForm orderDetailMgtForm_jtePopAddForm_form">
			...
			<colgroup>  <!-- 200507 원래는 col 4개에 각각의 width가 15 35 15 35였으나 col 6개로 변경_LYM -->
				...
				<!-- Lot코드 -->
					...
					<td colspan="2">
						<input type="text" id="lotId" class="left-input" name="lotId" />			<!-- 자동생성 -->
						<input type="hidden" id="lotSeq" class="left-input" name="lotSeq"/>
					</td>
					<!-- <td>
						<button class="k-button k-primary search-btn" type="button"  onclick="lfn_dupleCheck()">
						중복체크
						</button>
					</td> -->
				<!-- 관리자 -->
...
this.lfn_jtePop_set = function(mode,selectedItem){
	...
	if(mode=='NEW'){
		...
		gfn_resizePop(lfo_common.popId,{width:750,height:610});
...
this.lfn_jtePopGrd_popOpen = function(mode,selectedItem){
	var options = {
		...
		height: "440px",  ///200507 팝업창 height 변경_280->290px LYM 
		id:lfo_popGrd.popId, 
		//title:gfn_getMsg("pop_addPop"),			//추가팝업
		resizable:true,
		animation:{open :{effects:"expand:vertical fadeIn"},
...
this.lfn_jtePopGrd_save = function(mode){
	...
	var searchPrmt = {};
	searchPrmt.lotId = $('#lotId').val();
	console.log(mode);
	if(mode == "create"){
		$.ajax({
			async:false,
			url : lfo_common.ctrlUrl + '/selectDupleKeyCount' ,
			type: "POST",
			...
			success: function(data){
				if(data > 0) {
					kendo.confirm("중복 된 LotID가 존재합니다. 다른 ID를 입력하여 주십시오.")
					flag = true
				}
			},error: function(ex){
				gfn_loading(false);
