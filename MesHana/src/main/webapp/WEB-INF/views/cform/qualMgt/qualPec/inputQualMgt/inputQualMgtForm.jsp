		...
		<div class="pop_title">컨트롤번호 정보</div>
			...
			<colgroup>
				...
				<tr>
					...
					<td><input type="text" id="mtrlOrderQty" class="left-input" name="mtrlOrderQty" format="currency" readonly/></td>
					<td class="td-right-title"><i class="fas fa-caret-right popsc-left-caret"></i>검사수량 :</td>
					<td><input type="text" id="mtrlQualQty" class="right-input" name="mtrlQualQty" format="currency" readonly/></td>
				</tr>
				<tr class="input_hide">
					<td><i class="fas fa-caret-right"></i>불량수량 :</td>
					<td colspan="2"><input type="text" id="mtrlBadQty" class="left-input btn-input" name="mtrlBadQty" format="currency" readonly/></td>
				...
				<tr style="display:hidden;">
					<td>
						<input type="hidden" name="mtrlOrderId" id="mtrlOrderId"/>
...
this.lfn_jteInputQualAdd_setGrd = function(dataItem){
	...
	*/
	...
	lfo_inputQualAddGrd.crud  = {
		read:{url:"/getInputQualAddList", auth:"", prmt:{mtrlOrderId: dataItem.mtrlOrderId}
	...
	lfo_inputQualAddGrd.columns = [
		...
		{field: "mtrlQualQty", title:"검사수량", width: "110px"},
		{field: "mtrlPassQty", title:"합격수량", width: "110px"},
		{field: "mtrlBadQty", title:"불합격수량", width: "130px"},
...
this.lfn_inputQualPop_save = function(mode){
	...
	if(gfn_isEmpty(validation) || validation.validate()){
		...
		savePrmt.mtrlOrderId = $("#"+lfo_inputQualAddGrd.popId + " [name = mtrlOrderId]").val();
		savePrmt.mtrlQualQty = $('#mtrlOrderQty').val();
		savePrmt.mtrlPassQty = $('#mtrlPassQty').val();
		savePrmt.mtrlBadQty = $('#mtrlBadQty').val();
...
*/
	...
	//3.
	lfo_qualBadGrd.ctrlUrl = "/cform/qualMgt/infergodsMgt"
...
this.lfn_QualBadGrd_open = function(mode, dataItem){
	var options = {
		...
		height: "341px", // 200512 JJW 불량품 관리 팝업 높이 250px -> 330px_201120 YUMI 330->341
	...
	if(gfn_isEmpty(validation) || validation.validate()){
		...
		$.ajax({
			...
			success: function(data){
				...
				//불러온 데이터를 발주table에 저장
				var subSavePrmt = data.rows[0];
				subSavePrmt.action = 'U';
				$.ajax({
					async:false,
					url : lfo_inputQualAddGrd.ctrlUrl + lfo_inputQualAddGrd.crud.update.url,
					type: "POST",
					traditional :true,
					contentType : 'application/json',
					dataType : 'json',
					data: JSON.stringify(subSavePrmt),
					success: function(data){
						//마스터그리 리로드
						lfo_common.grid.dataSource.read();
						//팝업그리도 조회 
