...
*/
	...
	lfo_popGrd.columns = [
		...
		{field: "completeBtn", title:"납품완료", width:"150px",			//클레임정보 
			template : function(rows) {
				if(rows.lotState == "ord_sta01") {
					return '<button class="k-button k-primary" type="button" onclick = "lfn_complete_lot(this); " style="min-width:60px;">' + "납품완료" + '</button>';			//클레임 정보 입력
				} else {
					return '';
				}
			}
		...
		//2020.05.14|ymlee|LoT명 주석(신명기계 사용x)
		 {field: "lotNm", title:gfn_getMsg("col_lotWrkNm"), width: "200px;"},			//Lot명 
		...
		{field: "completeDate", title: "완료일자", width: "200px;"}			
	];
...
//LOT 완료 처리
this.lfn_complete_lot = function(selectedItem){
	
	var dataItem = lfo_popGrd.grid.dataItem($(selectedItem).closest("tr"));

	var savePrmt = {}
	savePrmt.lotId = dataItem.lotId;
	savePrmt.completeDate = "Complete";
	savePrmt.lotState = "ord_sta02";
	
	$.ajax({
		async:false,
		url : lfo_popGrd.ctrlUrl + "/setLotInfoComplete",
		type: "POST",
		data: JSON.stringify(savePrmt),
		traditional :true,
		contentType : 'application/json',
