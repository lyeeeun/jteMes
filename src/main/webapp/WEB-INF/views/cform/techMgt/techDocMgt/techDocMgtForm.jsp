...
<div id="grid-content" class="techDocMgtForm" style="width: 100%;">
	<div id="jteMasterTree" class="techDocMgtForm left-contents jteMasterTree"></div><input type="hidden" id = "jteMasterTree_hidSelected" class="jteMasterTree_hidSelected" value="">
...
<!-- 					<td colspan="3"><input id="docGroupTarget" name="docGroupTarget" format = "selBox" msg="docGroupTarget" class="jtePopForm_onlyInsert"/></td> -->
<!-- 				</tr> -->
				<tr>
				<!-- 기술문서 그룹 ID -->
					<td><i class="fas fa-caret-right"></i>그룹 ID : </td>
					<td colspan="3">
						<input type="text" id="docGroupId" name="docGroupId" class="jtePopForm_onlyInsert btn-input" />
						<input type="hidden" id="upDocGroupId" name="upDocGroupId"/>
						<input type="hidden" name="action" id="docGroupAction"/>
						<input type="hidden" id="docGroupTarget" name="docGroupTarget"/>
						<button class="k-button k-primary page-search-btn" type="button" id="itemSelBtn" onclick= "lfn_itemInfo()" style="display:none;"><spring:message code = "btn_search"/></button>
					</td>
				</tr>
				<tr>
				<!-- 기술문서 그룹 명 -->
				<td><i class="fas fa-caret-right"></i>그룹 명 :  </td>
					<td colspan="3">
						<input type="text" id="docGroupNm" name="docGroupNm" format="text"/>
					</td>
				</tr>
				<tr>
				<!-- 설명 -->
...
<div style="display:none;">
	<div id="techDocRegForm" class="techDocMgtForm">
		<form id="techDocRegForm_form" class="techDocRegForm techDocMgtForm" >
			<table style="width:100%;" id="techDocRegForm_master" class="BasicTable">
				<colgroup>
					<col style="width: 17%;">
					<col style="width: 32%;">
					<col style="width: 19%;">
				...
				<tr>
					...
					<td colspan="3">
						<input id="documentCd" name="documentCd" format="selBox" msg="techDocCd"/>
					</td>
				</tr>
				<tr>
				<!-- 기술문서 그룹 ID -->
					...
					<td><input type="text" id="documentRev" name="documentRev" /></td>
				</tr>
				<tr>
				<!-- 기술문서 그룹 명 -->
					<td><i class="fas  fa-caret-right"></i>문서 제목 :  </td>
					<td colspan="3"><input type="text" id="documentSubject" name="documentSubject" format="text"/></td>
				</tr>
				<!-- 설명 -->
				<tr>
					<td><i class="fas fa-caret-right"></i>비고 : </td>
					<td colspan="3"><input type="text" id="documentDesc" name="documentDesc" format="text"/></td>  <!-- style="width:95%" -->
				</tr>
				<!-- 파일 경로 -->
				<%-- <tr>
...
this.lfn_jteSubGrd_setGrd = function(){
	...
	$("#jteSubGrid").kendoGrid({
		dataSource: dataSource,
		columns: [
			{field: "techDocReg", title:"버튼", width: "230px",
				template : function(rows) {
					if(rows.docGroupTarget != "I") {
						return '<button class="k-button k-primary" type="button" onclick="lfn_techDocGrd_popOpen(this)">기술문서 등록</button><button class="k-button k-primary" type="button" onclick="lfn_jtePop_open(\'EDT\',this)">수정</button>';	
					} else {
						return '<span></span>';
					}
				}
			},
			{field: "docGroupId", title: "기술문서 그룹 ID", width: "160px"},
			{field: "docGroupNm", title: "기술문서 그룹 명", width: "150px"},
			{field: "docGroupTarget", title: "구분", width: "120px"},
		//	{field: "upDocGroupId", title:"상위 그룹 명", width: "200px"},
			...
			{field: "updatorNm", title:"수정자", width: "120px"}
...
this.lfn_jtePop_open = function(mode,selectedItem){
	...
	var options = {
			modal:true, 
			width: "600px", 
			height: "270px", /** 200806 메인 그리드에서 추가/수정 버튼 클릭 시 팝업 height 변경 350px->305px_youmi JJW 305 -> 340 **/
			id:"jtePopForm", 
			title:"", 
			workCd:mode, 
			bdSeq:"",
...
this.lfn_techDocRegPop_open = function(mode, rows){
	//초기화
 	var options = {
		modal: true, 
		width: "800px",
		height: "400px",  //201218 YUMI '기술문서 관리'인만큼 파일이 추가되어 있을 것으로 예상, 파일 1개 첨부되어 있는 기준으로 height 조정
