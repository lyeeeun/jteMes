...
this.lfn_chartDisplay = function() {
	...
	$.ajax({
		async:false,
		url : '/cform/perform/prodBadSta/getProdBad',
		type: "POST",
		...
		success: function(data){
			if(data.rows[0] == null) {
				...
				var output = data.rows[0].itemDefectRate;
				var test = data.rows[0].itemAchieveRate;
				var remain = data.rows[0].zeroRate;
				if(remain < 0) {
					remain = 0;
				}
			}
			...
			$("#performOrderChart").kendoChart({
	            ...
	            series: [{	
	                ...
	                data: [{
	                    category: "불량량",
	                    value: output,
	                    color:  "#FF3333" // 200501 JJW #2464ed 원형그래프 진한 색상 변경 LYM #2794c1
	                }, {
	                    category: "양품량",
	                    value: test,
	                    color: "#6666FF" // 200501 JJW #92B5FC 원형그래프 연한 색상 변경 LYM #299bc9
	                }, {
	                    category: "생산필요량",
	                    value: remain,
	                    color: "#99CCFF" // 200501 JJW #92B5FC 원형그래프 연한 색상 변경 LYM #299bc9
	                }]
	            }],
	            tooltip: {
	                visible: true,
