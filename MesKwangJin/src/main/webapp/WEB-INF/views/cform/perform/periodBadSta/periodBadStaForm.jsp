...
this.lfn_jteSgGrd_setGrd = function(target){
	...
	$("#jteSingleGrid").kendoGrid({
			...
			columns: [
					{ field: "itemId", title: gfn_getMsg("col_itemCd", "제품코드"), width: "10%" },
					{ field: "itemNm", title: gfn_getMsg("col_itemNm", "제품명"), width: "10%" },
					{ field: "itemOrderQty", title: gfn_getMsg("col_totalOrderQty", "총주문수량"), width: "10%" },
					{ field: "itemDefectQty", title: gfn_getMsg("col_badProdQuan", "불량수량"), width: "10%" },
					{ field: "itemDefectRate", title: gfn_getMsg("col_badRate", "불량률"), format: "{0:0.000'%'}", width: "10%" }
			],
		...
		$("#searchDate").kendoDatePicker({
			...
			change: function() {
				...
				$("#jteSingleGrid").data("kendoGrid").dataSource.query({
					sort: [{'field':'itemDefectQty', 'dir':"desc"}]
				});
