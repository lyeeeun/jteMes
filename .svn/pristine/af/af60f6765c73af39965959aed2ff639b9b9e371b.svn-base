...
public class NewMtrlInfoService {
	...
	public PageInfo<NewMtrlInfoDto> getMtrlInfoList(NewMtrlInfoDto mtrlInfoDto, PageRequestVo pageRequestVo){
		...
		try {
			//Count
			rowCount = newMtrlInfoMapper.selectMtrlInfoCount(mtrlInfoDto);
			if (rowCount > 0) {
				//List
				mtrlInfoList = newMtrlInfoMapper.selectMtrlInfoList(mtrlInfoDto);
			}
		} catch (Exception e) {
	...
	//자재정보 입력/수정
	@Transactional
	public String setMtrlInfoSave(NewMtrlInfoDto mtrlInfoDto){
		String returnKey = "";
		try {
			...
			if(mtrlInfoDto.getAction().equals("C")) {//
				//자재입력
				newMtrlInfoMapper.insertMtrlInfo(mtrlInfoDto);
				...
				//자재수정
				newMtrlInfoMapper.updateMtrlInfo(mtrlInfoDto);
	...
	public int setMtrlInfoDelete(List<NewMtrlInfoDto> mtrlInfoList){
		...
		try {
			//자재삭제
			for(NewMtrlInfoDto mtrlInfoDto : mtrlInfoList){
				newMtrlInfoMapper.deleteMtrlInfo(mtrlInfoDto);
			}
		} catch (Exception e) {
			resultInt = -1;
	...
	public int setMtrlRtlCompDelete(List<NewCompMgtDto> compList){
		...
		try {
			...
			for(NewCompMgtDto compDto : compList){
				newMtrlInfoMapper.deleteMtrlRtlComp(compDto);
			}
		} catch (Exception e) {
			resultInt = -1;
