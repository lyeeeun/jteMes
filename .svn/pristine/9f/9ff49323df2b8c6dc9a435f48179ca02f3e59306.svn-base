...
public class NewPerformEquipmentService {
	...
	public PageInfo<NewPerformEquipmentDto> getPerfromEquipInfo(NewPerformEquipmentDto newPerformEquipmentDto, PageRequestVo pageRequestVo){
		
		List<NewPerformEquipmentDto> performEquipInfoList = null;
		int rowCount = 0;
		if(!newPerformEquipmentDto.getPagingYN().equals("N")) {
			newPerformEquipmentDto.setFirstIndex(pageRequestVo.getBlockFirstPage());
			newPerformEquipmentDto.setLastIndex(pageRequestVo.getBlockLastPage());
		}
		
		try {
			rowCount = newPerformEquipmentMapper.rowCount(newPerformEquipmentDto);
			if (rowCount > 0) {
				performEquipInfoList = newPerformEquipmentMapper.selectPerformEquipInfo(newPerformEquipmentDto);
			}
		} catch (Exception e) {
