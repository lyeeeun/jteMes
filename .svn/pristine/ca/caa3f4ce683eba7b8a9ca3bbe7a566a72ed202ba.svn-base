...
<%-- 			<tr>
				...
				<!-- 자재명 -->
					...
					<td colspan="2">
						<input type="text" id="mtrlNm" class="left-input" name="mtrlNm" format="text" readonly />
					</td>
				<!-- 자재구분 -->
...
<%-- 					<td><spring:message code = "col_prescripPeriod"></spring:message> : </td>	<!-- 시효기간 -->
					<td><input id="mtrlMgtPrescription" name="mtrlMgtPrescription" format="datepicker" style="width: 95%;" /></td> --%>
	...
	lfo_common.customTitle = {
		html :	'<div style="margin : 5px 0 5px 0;">반납여부 : '+
				'<input type="radio" name="delChk" id="delFalse" class="k-radio" checked="checked" value=false>' +
			 	'<label for="delFalse">'+gfn_getMsg("col_stock")+'</label>'+
				'<input type="radio" name="delChk" id="delTrue" class="k-radio"  value=true>'+
				'<label for="delTrue">'+gfn_getMsg("col_mtrlReturn")+'</label>'+
...
/* 				var preScriptionDate = $("#mtrlMgtPrescription").data("kendoDatePicker");
		...
		//그리드 세팅
		gfn_resizePop(lfo_common.popId, {width:750,height:600});
		$("#mtrlQtyArea").show();
...
this.lfn_jtePopGrd_popOpen = function(mode,selectedItem){
	var options = {
		...
		height: "270px",   //200512_마스터그리드 > 팝업창 내부 그리드 > 더블클릭 시 나타나는 팝업 높이_160px -> 245px LYM 200519 JJW 160-> 270
		id:lfo_popGrd.popId, 
		//title:gfn_getMsg("pop_addPop"),
		resizable:true,
		animation:{open :{effects:"expand:vertical fadeIn"},
