		...
		try {
			...
			if(newLotDto.getAction().equals("C")) {
				...
				if(!StringUtils.isEmpty(newLotDto.getMtrlId())) {
					NewMtrlHistoryDto mtrlCntDto = new NewMtrlHistoryDto(newLotDto.getMtrlId(), "mtrl_tagt05", newLotItemDto.getLotId(),
...
//				newItemMapper.updateItemDeduction(newItemMgtDto);
	...
	@Transactional
	public void setLotInfoComplete(NewLotInfoDto newLotDto){
		try {
			newLotDto.setUpdatorId(JteUtils.getUserId());
			newOrderMapper.lotComplete(newLotDto);
		} catch (Exception e) {
			throw new BusinessException("setLotInfoComplete : " + e.getMessage());
		}
	}
}
