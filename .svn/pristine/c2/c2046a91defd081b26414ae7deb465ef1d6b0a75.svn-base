...
<div style="width:100%;">
	<div class="shipPlanMgtForm search-btn-area" style="width:100%; background:#f0f5ff; padding:15px 0 15px 0; margin:10px 0 5px 0;">
		<label>프로젝트 : </label>
...
this.lfn_jteSgGrd_setGrd = function(){
	...
	lfo_common.columns = [
		{field: "shipPlan", title: gfn_getMsg("col_shipPlan"), width:"120px",												//포장작업
			template : function(rows) {
				if(rows.shipState != "ship_dvry02" && rows.shipState != "ship_dvry03") {
					return '<button class="k-button k-primary" type="button" onclick="lfn_shipPlanPop_open(this)">출하 계획</button>';
				} else {
					return '<span></span>'
				}
			}
		...
		{field: "prsPackageQty", title:gfn_getMsg("col_bundleQty"), width: "110px;"},										//포장 수량
...
this.lfn_saveShipPlan = function() {
	...
	if($('#shipId').val() == null || $('#shipId').val() == undefined || $('#shipId').val() == "") {
		$.ajax({
			async:false,
			url : "/form/ship/shipPlanMgt/setShipPlan",
			type: "POST",
		...
		$.ajax({
			async:false,
			url : "/form/ship/shipPlanMgt/updateShipPlan",
			type: "POST",
			data: JSON.stringify(savePrmt),
			traditional :true,
			contentType : 'application/json',
