...
public class MtrlInfoController extends BaseController {
	...
	public ModelMap getMtrlPop(ModelMap model, @ModelAttribute("mtrlInfoDto") MtrlInfoDto mtrlInfoDto) {
		try {
			model.addAttribute("rows", mtrlInfoService.getMtrlPop(mtrlInfoDto)); 
		}catch(Exception ex) {
...
//		mtrlInfoService.minOutOfStockAlarm(mtrlMgtId);
//	}
