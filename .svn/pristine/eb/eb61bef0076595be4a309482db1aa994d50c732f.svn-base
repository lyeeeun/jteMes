...
<div id="grid-content" class="infergodsMgtForm" style="width:100%;">
	<div class="select-btn-area">
		<input class="radio-btn" type="radio" name="target" value="item" checked><span class="radio-span">부품</span>
		<input class="radio-btn" type="radio" name="target" value="mtrl"><span class="radio-span">자재</span>
		<input class="radio-btn" type="radio" name="target" value="eqmt"><span class="radio-span">설비</span>
		<input class="radio-btn" type="radio" name="target" value="worker"><span class="radio-span">작업자</span>
	</div>
	<div class="infergodsMgtForm-gird">
...
this.lfn_jteSgGrd_setGrd = function(gubun){
	...
	*/
	...
	if(gubun == "item") {
		...
		lfo_common.selectBox = [
			{ text: gfn_getMsg("col_mtrlDiv"), value: "mtrlQty.description" }			//자재구분
		];
	} else if(gubun == "eqmt") {
		lfo_common.model = {
			id: "eqmtMgtId",
