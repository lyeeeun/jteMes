...
import jin.mes.form.mfgMgt.wrkinTeamMgt.WrkinTeamMgtMapper;
import jin.mes.form.mfgMgt.wrkinWrkerMgt.WrkinWrkerMgtMapper;
								...
								if(calStdt.get(Calendar.DAY_OF_WEEK) != 1) {//일요일 제외
									...
									if(Arrays.asList(productPlanDto.getBulkTarget()).contains("asgn")) {
										...
										for(WrkinTeamMgtDto asgnDto : asgnList) {
											...
											//생산지시 생성자
											WrkinTeamMgtDto insertAsgnDto = new WrkinTeamMgtDto(null, asgnDto.getProdAsmNm(), strStdt, String.valueOf(planCnt), 
