...
import jin.mes.form.basMgt.userMgt.userInfo.UserInfoMapper;
...
//	@Override
	...
	public UserInfoVo findByUserId(String userId, boolean del) {
		return userRepository.findByUserIdAndIsDel(userId, del);
	}
//	@Override
    @Transactional
...
//	@Override
	...
	public void setFailCntByUserId(String userId) {
		userRepository.updateFailCntByUserId(userId);
		
		UserInfoVo userInfo = userRepository.findByUserId(userId);
		if(userInfo.getFailCnt() >= 4) {
			userRepository.updateIsLock(userId);
		}
	}
	
	
	public void setFailCntInitByUserId(String userId) {
		try {
//			UserInfoVo userInfo = userRepository.findByUserId(userId);
//			userInfo.setFailCnt(0);
//			userRepository.save(userInfo);
			/*** Transactional 관련해서 확인 필요. 어노테이션으로 해결 안됨.. 따라서 jpa가 아닌 mybatis 사용 ***/
			UserInfoDto userInfoDto = new UserInfoDto();
			userInfoDto.setUserId(userId);
			userInfoDto.setUpdatorId(userId);
