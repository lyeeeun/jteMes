...
<%-- 			<tr>
				...
				<!-- 자재코드 -->
					...
					<td>
						<input type="text" id="mtrlId" class="btn-input" name="mtrlId"  placeholder="<spring:message code = "pop_searchAuto"></spring:message>" readonly />			<!-- 조회시자동입력 -->
						<button class="k-button k-primary" type="button" id="mtrl_search_btn" onclick= "lfn_mtrlInfoPop_Open();" style="display:none;"><spring:message code = "btn_search"/></button>
					</td>
				<!-- 자재유형 -->
...
<%-- 				<tr>
				...
				<!-- 자재개별코드 -->
					...
					<td><input type="text" id="mtrlMgtId" name="mtrlMgtId" placeholder="<spring:message code = "pop_autoInput"></spring:message>" readonly /></td>			<!-- 자동입력 -->
					<!-- 자재수량 -->
...
<!-- 					<td colspan="2"><input id="lotId" class="right-input" name="lotId" readonly /></td> -->
				...
				<%-- <tr>
					...
					<!-- 04.29/ymlee/신명기계 시효기간 미사용으로 주석 -->
 					<td class="td-right-title"><i class="fas fa-caret-right left-caret" ></i><spring:message code = "col_prescripPeriod"></spring:message> : </td>	<!-- 시효기간 -->
					<td colspan="2"><input id="mtrlMgtPrescription" class="right-input" name="mtrlMgtPrescription" format="datepicker"/></td> 
				</tr> --%>
				<tr>
				<!-- 설명 -->
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_desc"></spring:message> : </td>
	...
	<div id="jtePopAddForm">
		<form id = "jtePopAddForm_form" class="mtrlMgtForm">
			...
			<colgroup>  <!-- 200511 원래는 23 27 23 27_LYM -->
				...
				<!-- 등록자 -->
					<td class="td-right-title"><i class="fas fa-caret-right"></i><spring:message code = "col_registUser"></spring:message> : </td>
					<td>
				...
				<!-- 프로그램명 -->
					<td class="td-right-title"><i class="fas fa-caret-right"></i><spring:message code = "col_progNm"></spring:message> : </td>
...
/* 		{field : "mtrlHCD",  template:"# var item = gfn_isNull(gfn_getCode(mtrlHCD))== true ? '기초코드 없음' : gfn_getMsg('bc_'+ gfn_getCode(mtrlHCD).cdId , gfn_getCode(mtrlHCD).cdNm) # #= item #",
		...
		{field : "mtrlMgtCnt", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_mtrlQty"), width: "110px"},								//자재수량
		{field: "mtrlStd04", title: "최소재고",   attributes: { style: "color: red; font-weight:bold;  text-align:right;" }, width: "105px"},
		//{field : "mtrlMgtScrap", format: "{0:n0}", attributes : { style : "text-align : right;"}, title : gfn_getMsg("col_mtrlScrap"), width: "110px"},						//잔량
...
// 					return '반납완료';
// 				}else{
// 					return '<button class="k-button k-primary" type="button" onclick = "lfn_mtrl_return(this); " style="min-width:90px;">' + "자재반납" + '</button>';
// 				}
// 			}
// 		}
	];
...
*/
	...
	lfo_popGrd.columns = [
		...
		{field: "mtrlQtyTargetCode",template:'<button class="k-button k-primary pop-off" type="button" onclick = "lfn_jtePopGrd_popOpen(\'EDT\',this); " style="min-width:60px;">' + "수정" + '</button>',
			title : gfn_getMsg("col_update"), width: "80px"},	/* 1214 JJW 120 -> 90 */														//수정
		...
		{field: "mtrlQtyTotal", format:"{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_quantity"), width: "110px"},						//수량
...
this.lfn_jtePopGrd_popOpen = function(mode,selectedItem){
	var options = {
		...
		height: "285px",   /* 20.12.18 JJW 285 -> 305 */
		id:lfo_popGrd.popId, 
		//title:gfn_getMsg("pop_addPop"),
		resizable:true,
		animation:{open :{effects:"expand:vertical fadeIn"},
