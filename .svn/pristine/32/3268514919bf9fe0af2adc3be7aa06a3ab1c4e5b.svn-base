package jin.mes.cform.basMgt.operMgt.facilMgt.sinMyung;
...
//@Service
public class NewFacilMgtServiceSinMyung {
	...
	//설비상세 조회
	public PageInfo<NewFacilMgtDto> getEqmtMgtList(NewFacilMgtDto eqmtMgtDto, PageRequestVo pageRequestVo){
		...
		try {
			//Count
			rowCount = newFacilMgtMapper.selectEqmtMgtCount(eqmtMgtDto);
			if (rowCount > 0) {
				//List
				eqmtMgtList = newFacilMgtMapper.selectEqmtMgtList(eqmtMgtDto);
			}
		} catch (Exception e) {
	...
	//설비상세 조회(ALL)
	public List<NewFacilMgtDto> getEqmtMgtListAll(NewFacilMgtDto eqmtMgtDto){
		List<NewFacilMgtDto> eqmtMgtList = null;
		try {
			//List
			eqmtMgtList = newFacilMgtMapper.selectEqmtMgtListAll(eqmtMgtDto);
		} catch (Exception e) {
	...
	//설비 상세 입력/수정
	@Transactional
	public String setEqmtMgtSave(NewFacilMgtDto eqmtMgtDto){
		String returnKey = "";
		try {
			...
			if(eqmtMgtDto.getAction().equals("C")) {//설비상세  등록
				...
				//설비상세 입력
				newFacilMgtMapper.insertEqmtMgt(eqmtMgtDto);
				...
				//설비상세 수정
				newFacilMgtMapper.updateEqmtMgt(eqmtMgtDto);
	...
	public int setEqmtMgtDelete(List<NewFacilMgtDto> eqmtMgtList){
		...
		try {
			//설비상세 삭제
			for(NewFacilMgtDto eqmtMgtDto : eqmtMgtList){
				newFacilMgtMapper.deleteEqmtMgt(eqmtMgtDto);
			}
		} catch (Exception e) {
			resultInt = -1;
