	...
	<div id="jtePopForm" class="userInfoForm-div">
		<form id="jtePopForm_form" class="userInfoForm">
			...
			<tr><td colspan="4"></td></tr>			
				...
				<tr>
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_jobOrderBasic"/> : </td>
					<td colspan="3">
						<input id="userStrStd01" class="center-input" name="userStrStd01" format = "selBox" custom="true" keyGbn="placeId" keyText="placeNm" func="lfn_placeParent_get()" />
					</td>
				</tr>
				<tr>
				<!-- 설명 -->
	...
	*/
	...
	lfo_common.model = {
		...
		fields: {
			...
			userStrStd01: { type: "string" },
			userStrStd02: { type: "string" },
			userStrStd03: { type: "string" },
	...
	//6.
	lfo_common.columns = [
		{field: "userId",template:'<button class="k-button k-primary" type="button" onclick = "lfn_jtePop_open(\'EDT\',this); " style="min-width:60px;">' + gfn_getMsg("pop_view") + '</button>',
			title:gfn_getMsg("col_update"), width: 60},
		...
		{field: "placeNm", title:gfn_getMsg("col_jobOrderBasic"), width: "100px"},										//작업지시서 기본
...
this.lfn_placeParent_get = function(){
	var placeList = gfn_ajaxCallByList("place",{placeStrStd01 : "ParentWorkLine"});
	placeList.unshift({placeId:"ALL",placeNm:"전체"});
	return placeList;
}
