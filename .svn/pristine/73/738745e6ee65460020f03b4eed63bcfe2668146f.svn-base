...
<%-- 				<tr class="pop_detail_info">
				...
				<tr class="pop_detail_info">
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_remark"></spring:message> : </td>
...
this.lfn_jteSgGrd_setGrd = function(){
	...
	*/
	...
	lfo_common.customTitle = {
		html :	'<div style="margin : 5px 0 5px 0;">'+'<i class="grid-radio-title fas fa-caret-right"></i>'+'<span style="font-weight: bold;"> '+ gfn_getMsg("col_checkReturn")+ ' :'+'</span>'+
				'<input type="radio" name="delChk" id="delFalse" class="k-radio" checked="checked" value=false style="margin : 0 5px 0 5px;">' +
			 	'<label class="itemStockMgtForm-label" for="delFalse">'+gfn_getMsg("col_useAvailable")+'</label>'+
				'<input type="radio" name="delChk" id="delTrue" class="k-radio"  value=true style="margin : 0 5px 0 5px;">'+
				'<label class="itemStockMgtForm-label" for="delTrue">'+gfn_getMsg("col_useComplete")+'</label>'+
	...
	lfo_common.columns = [
		...
		{field: "itemPackageStock", format: "{0:n0}", title:gfn_getMsg("col_releaseQty"), width: "110px"},												//출고 재고 수량
		{field: "description", title:gfn_getMsg("col_desc"), width: "240px"},
		{field : "isDel",title : gfn_getMsg("col_useComplete"), width: "120px",
			template : function(rows){
				if(rows.del){
					return gfn_getMsg("btn_useComplete");
				}else{
					return '<button class="k-button k-primary" type="button" onclick = "lfn_item_completeUse(this); " style="min-width:90px;">' + gfn_getMsg("btn_useComplete") + '</button>';
				}
			}
		} // 사용 완료
	];
