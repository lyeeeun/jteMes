...
<!-- 전주원 주임 작업 CSS -->
	<link href="<c:url value='/resources/mes/css/fragments/left_jjw.css'/>" rel="stylesheet">
	<link href="<c:url value='/resources/mes/css/fragments/header_jjw.css'/>" rel="stylesheet">
<script>
	...
	$(document).ready(function(){
		...
		$("#toolbar").kendoToolBar({
			items: [
				...
				{ template: "<img src='resources/mes/img/HANAITM_logo.png' style='margin-left: 20px; height:35px; margin:5px; margin-top: 7px;'>" }, /* template: "<img src='resources/mes/img/sinmyeong_logo_png.png' style='margin-left: 20px; height:35px; margin:5px;'>" 여기서 로고 수정 */
		...
		template: "<div class='left-area'>"+
			...
			"<div id='menuPathNaviTop'>Mes / <span>Loading...</span></div>"+
		"</div>",
		position: 'left',
		//모바일에서는 제거
		mode:"push",
		...
		//penelBar에 들어갈 메뉴 데이터 생성 - tree구조 데이터
		<c:forEach items="${principal.myMenuVos}" var="menu" varStatus="status">
			//폴더인 경우 아이콘	
			...
			//메뉴 리스트에 Hidden 처리 할 항목 -> DB에 설정 값 적용 혹은 기초코드 이용하여 처리 예정 
			if("${menu.menuId}" != "cpkExcel" && "${menu.menuId}" != "cpkOverviewDetail") {
...
<!-- 	최상단 -->
	<div class="my-config-area" >
		<ul id = "user-config-dropdown">
			<li>
				<span><spring:message code = "top_greet"></spring:message> </span>																			<!-- 안녕하세요 -->
		...
		<spring:eval expression="T(kr.co.itcall.jte.common.util.CodeUtil).getCode('sys.flag.reflash', 600)" var="cacheTimeout" />
	</div>
	<!-- 	툴바	 -->
	<div id ="toolbar"></div>
	<!-- 	좌측메뉴 -->
