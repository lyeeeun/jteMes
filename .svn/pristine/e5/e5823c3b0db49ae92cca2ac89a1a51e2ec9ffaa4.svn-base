		...
		<form id="jtePopForm_form" class="mtrltoolMgtForm">
			...
			<colgroup>
				...
				<!-- 공구개별코드 -->
					...
					<td colspan="3"><input type="text" id="toolMgtId" class="center-input" name="toolMgtId"/></td>
				</tr>
				<tr>
				<!-- 공구명 -->
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_toolNm"></spring:message> : </td>
					<td colspan="3"><input type="text" id="toolNm" class="center-input" name="toolNm" readonly/></td>
				</tr>
				<tr>
				<!-- 공구상태 -->
...
this.lfn_jteSgGrd_setGrd = function(){
	...
	*/
	...
	//6.
	lfo_common.columns = [
		{field: "toolId",template:'<button class="k-button k-primary" type="button" onclick = "lfn_jteSgGrid_dblclick(\'EDT\',this); " style="min-width:60px;"><spring:message code = "col_detail"></spring:message></button>',
...
this.lfn_jteSubGrd_setGrd = function(){
	...
	*/
	...
	//3.
	lfo_subGrd.ctrlUrl = "/cform/mtrl/mtrltoolMgt";
	...
	lfo_subGrd.customTitle = {
		html :	'<div style="margin : 5px 0 5px 0;"><i class="grid-radio-title fas fa-caret-right"></i><span style="font-weight: bold;"> <spring:message code = "col_availability"></spring:message> : </span>'+
				'<input type="radio" name="delChk" id="delFalse" class="k-radio" checked="checked" value=false style="margin : 0 5px 0 5px;">' +
			 	'<label class="mtrltoolMgtForm-label" for="delFalse"><spring:message code = "bc_isUse_Y"></spring:message></label>'+
				'<input type="radio" name="delChk" id="delTrue" class="k-radio"  value=true style="margin : 0 5px 0 5px;">'+
				'<label class="mtrltoolMgtForm-label" for="delTrue"><spring:message code = "bc_isUse_N"></spring:message></label>'+
	...
	lfo_subGrd.columns = [
		...
		{field: "toolMgtId",template:'<button class="k-button k-primary" type="button" onclick = "lfn_jtePop_open(\'EDT\',this); " style="min-width:60px;"><spring:message code = "col_update"></spring:message></button>',
			title : gfn_getMsg("btn_search") , width: "110px"},																//조회
		...
		{field : "toolMgtId",title : gfn_getMsg("col_mtrlReturn"), width: "120px",											//반납
			template : function(rows){
				if(rows.del){
					return '<spring:message code = "col_discarded"></spring:message>';
				}else{
					return '<button class="k-button k-primary" type="button" onclick = "lfn_tool_return(this); " style="min-width:90px;"><spring:message code = "col_toolDisposal"></spring:message></button>';
				}
			}
		}
	];
	...
	lfo_subGrd.validation ={
		...
		toolMgtPurchase:{
			messages : gfn_getMsg("pop_petpud"),
			rules : function(input){
...
this.lfn_jtePop_open = function(mode, selectedItem){
	var options = {
		modal:true, 
		width: "630px", 
		height: "460px", // 200511 JJW 오른쪽 그리드 - 추가버튼 팝업 높이 기본값 330px -> 420px _ LYM 330 -> 415 200810 youmi 420->425px 201020 youmi 445->460
		id:lfo_subGrd.popId, 
		//title:"추가팝업",
		resizable : true,
...
this.lfn_popSave = function(mode){
	...
	var searchPrmt = {};
	searchPrmt.toolMgtId = $('#toolMgtId').val();
	
	if(mode == "NEW"){
		$.ajax({
			async:false,
			url : lfo_subGrd.ctrlUrl + '/selectToolMgtDupleKeyCount' ,
			type: "POST",
			...
			success: function(data){
				if(data > 0) {
					console.log(data);
					kendo.confirm("중복 된 공구개별코드가 존재합니다. 다른 ID를 입력하여 주십시오.")
					flag = true
				}
			},error: function(ex){
				gfn_loading(false);
...
this.lfn_jteToolMtnc_setGrd = function(dataItem){
	...
	*/
	...
	//3.
	lfo_toolMtncGrd.ctrlUrl = "/cform/mtrl/mtrltoolMgt";
...
this.lfn_eqmtMtncPop_open = function(mode, selectedItem){
	var options = {
		...
		height: "335px", // 200511 JJW 오른쪽 그리드 정비이력 버튼 클릭 - 그리드 추가버튼 클릭 시 나타나는 팝업 높이 230px -> 320px _LYM 230px -> 328px // 201020 320->335  
...
this.lfn_jteToolUse_setGrd = function(dataItem){
	...
	*/
	...
	//3.
	lfo_toolUseGrd.ctrlUrl = "/cform/mtrl/mtrltoolMgt";
