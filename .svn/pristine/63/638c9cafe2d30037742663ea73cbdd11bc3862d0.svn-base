	...
	if(mode=='NEW'){
		...
		$.ajax({
			...
			success: function(data){
				...
				$("#langCd").data("kendoDropDownList").select(function(dataItem) {
					return dataItem.cdVal === data.rows[0].langCd;
				});
			},error: function(ex){
				gfn_loading(false);
	...
	$.ajax({
		...
		success: function(data){
			...
			$(data).each(function(index,item){
				...
				authHTML+='<tr>'
					+'<td>'
						+'<input type="checkbox" name="role" id="'+item.roleId+'" class="k-checkbox" '+isChk+' >'
						+'<label for="'+item.roleId+'" value ="'+item.roleId+'">'+item.roleNm+'</label>'
					+'</td>'
				+'</tr>';
