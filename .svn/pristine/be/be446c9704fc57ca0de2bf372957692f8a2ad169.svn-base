...
<script>
	...
	$(document).ready(function(){
		if(gfn_getCode("alarmYN").cdVal == "Y"){
			//알람 최소 생성(1분마다 리로드)
			gfn_getMyAlarm();
			setInterval(function(){gfn_getMyAlarm()},60 * 1000)
			$("#alarmArea").show();
		}
		//좌측메뉴 툴바
		$("#toolbar").kendoToolBar({
			items: [
				...
				{ template: "<img src='resources/mes/img/LSK_logo.png' style='margin-left: 20px; height:35px; margin:5px; margin-top: 7px;'>" }, /* template: "<img src='resources/mes/img/sinmyeong_logo_png.png' style='margin-left: 20px; height:35px; margin:5px;'>" 여기서 로고 수정 */
		...
		//penelBar에 들어갈 메뉴 데이터 생성 - tree구조 데이터 - -여기부터 2021-03-09
