...
public class NewOrderDetailMgtController extends BaseController {
	...
	@RequestMapping("getMidLotList")
	public ModelMap getMidLotList(ModelMap model, @ModelAttribute("newLotInfoDto") NewLotInfoDto newLotInfoDto) {
		PageRequestVo pageRequestVo = new PageRequestVo();
		pageRequestVo.setPage(getCurrentPage(Integer.toString(newLotInfoDto.getPage())));
		pageRequestVo.setRows(newLotInfoDto.getRows());
		
		try {
			setModelWithkendoList(model, newOrderDetailMgtService.getMidLotList(newLotInfoDto,pageRequestVo));
		}catch(Exception ex) {
	...
	@RequestMapping("/setMidLotInfoSave")
	@ResponseBody
	public String setMidLotInfoSave(@RequestBody NewLotInfoDto newLotInfoDto) {
		return newOrderDetailMgtService.setMidLotInfoSave(newLotInfoDto);
	}
	
	@RequestMapping("/setMidLotInfoDelete")
	@ResponseBody
	public int setMidLotInfoDelete(ModelMap model, @RequestBody List<NewLotInfoDto> lotList) {
		return newOrderDetailMgtService.setMidLotInfoDelete(lotList);
	}
	
	@RequestMapping("/selectMidlotDupleKeyCount")
	@ResponseBody
	public int selectMidlotDupleKeyCount(@RequestBody NewLotInfoDto newLotInfoDto) {
		return newOrderDetailMgtService.selectMidlotDupleKeyCount(newLotInfoDto);
	}
	
	
	@RequestMapping("getLotList")
	public ModelMap getLotList(ModelMap model, @ModelAttribute("newLotInfoDto") NewLotInfoDto newLotInfoDto) {
		PageRequestVo pageRequestVo = new PageRequestVo();
		pageRequestVo.setPage(getCurrentPage(Integer.toString(newLotInfoDto.getPage())));
		pageRequestVo.setRows(newLotInfoDto.getRows());
	...
	@RequestMapping("/selectLotDupleKeyCount")
	@ResponseBody
	public int selectLotDupleKeyCount(@RequestBody NewLotInfoDto newLotInfoDto) {
		return newOrderDetailMgtService.selectLotDupleKeyCount(newLotInfoDto);
	}
}
