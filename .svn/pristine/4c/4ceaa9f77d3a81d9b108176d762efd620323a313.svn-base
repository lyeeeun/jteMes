...
public class NewMtrltoolMgtService {
	...
	// 공구상세 조회
	public PageInfo<NewMtrltoolMgtDto> getToolMgtList(NewMtrltoolMgtDto toolMgtDto, PageRequestVo pageRequestVo) {
		...
		try {
			// Count
			rowCount = newMtrltoolMgtMapper.selectToolMgtCount(toolMgtDto);
			if (rowCount > 0) {
				// List
				toolMgtList = newMtrltoolMgtMapper.selectToolMgtList(toolMgtDto);
			}
		} catch (Exception e) {
	...
	// 공구 상세 입력/수정
	@Transactional
	public String setToolMgtSave(NewMtrltoolMgtDto toolMgtDto) {
		String returnKey = "";
		try {
...
//				}
				...
				// 공구상세 입력
				newMtrltoolMgtMapper.insertToolMgt(toolMgtDto);
				...
				// 공구상세 수정
				newMtrltoolMgtMapper.updateToolMgt(toolMgtDto);
				...
				newMtrltoolMgtMapper.updateToolMgtIsDel(toolMgtDto);
				returnKey = toolMgtDto.getToolMgtId();
			}
...
//			}
	...
	public int setToolMgtDelete(List<NewMtrltoolMgtDto> toolMgtList) {
		...
		try {
			// 공구상세 삭제
			for (NewMtrltoolMgtDto toolMgtDto : toolMgtList) {
				newMtrltoolMgtMapper.deleteToolMgt(toolMgtDto);
			}
		} catch (Exception e) {
			resultInt = -1;
	...
	// 공구정비이력 조회
	public PageInfo<NewMtrltoolMgtDto> getToolMtncList(NewMtrltoolMgtDto toolMgtDto, PageRequestVo pageRequestVo) {
		List<NewMtrltoolMgtDto> toolMgtList = null;
		int rowCount = 0;
		...
		try {
			// Count
			rowCount = newMtrltoolMgtMapper.selectToolMtncCount(toolMgtDto);
			if (rowCount > 0) {
				// List
				toolMgtList = newMtrltoolMgtMapper.selectToolMtncList(toolMgtDto);
			}
		} catch (Exception e) {
	...
	// 공구 정비이력 입력/수정
	@Transactional
	public String setToolMtncSave(NewMtrltoolMgtDto toolMgtDto) {
		String returnKey = "";
		try {
			toolMgtDto.setCreatorId(JteUtils.getUserId());
			toolMgtDto.setUpdatorId(JteUtils.getUserId());
			...
			if (toolMgtDto.getAction().equals("C")) {// 공구정비이력 등록
				...
				// 공구정비이력 입력
				newMtrltoolMgtMapper.insertToolMtnc(toolMgtDto);
...
//				toolMgtDto.setToolMgtVerif(df.format(cal.getTime()));
				...
				// 공구정비이력 수정
				newMtrltoolMgtMapper.updateToolMtnc(toolMgtDto);
	...
	public int setToolMtncDelete(List<NewMtrltoolMgtDto> toolMgtList) {
		...
		try {
			// 공구정비이력 삭제
			for (NewMtrltoolMgtDto toolMgtDto : toolMgtList) {
				newMtrltoolMgtMapper.deleteToolMtnc(toolMgtDto);
			}
		} catch (Exception e) {
			resultInt = -1;
	...
	// 공구사용정보 조회
	public PageInfo<NewMtrltoolMgtDto> getToolUseList(NewMtrltoolMgtDto toolMgtDto, PageRequestVo pageRequestVo) {
		List<NewMtrltoolMgtDto> toolMgtList = null;
		int rowCount = 0;
		...
		try {
			// Count
			rowCount = newMtrltoolMgtMapper.selectToolUseCount(toolMgtDto);
			if (rowCount > 0) {
				// List
				toolMgtList = newMtrltoolMgtMapper.selectToolUseList(toolMgtDto);
			}
		} catch (Exception e) {
	...
	public int getToolUseChangeTotal(NewMtrltoolMgtDto toolMgtDto) {
		int intResult = 0;
		try {
			intResult = newMtrltoolMgtMapper.selectToolUseChangeTotal(toolMgtDto);
		} catch (Exception e) {
			throw new BusinessException("공구사용정보 조회 에러입니다.(List) : " + e.getMessage());
		}
		return intResult;
	...
	public String setToolUsePgSave(NewMtrltoolMgtDto toolMgtDto) {
		try {
			//등록자/수정자 등록
			toolMgtDto.setCreatorId(JteUtils.getUserId());
			toolMgtDto.setUpdatorId(JteUtils.getUserId());
			...
			if(toolMgtDto.getAction().equals("C")) {
				//공구 사용이력 - 등록
				newMtrltoolMgtMapper.insertToolUse(toolMgtDto);
			} else if(toolMgtDto.getAction().equals("U")){
				//공구 사용이력 - 수정
				newMtrltoolMgtMapper.updateToolUse(toolMgtDto);
			}else if(toolMgtDto.getAction().equals("D")){
				//공구 사용이력 - 삭제
				newMtrltoolMgtMapper.deleteToolUse(toolMgtDto);
			}
			
			//공구관리 - 수량 업데이트
			toolMgtDto.setToolMgtUse(newMtrltoolMgtMapper.selectToolUseTotal(toolMgtDto));
			newMtrltoolMgtMapper.updateToolMgt(toolMgtDto);
	...
	//공구사용 수정 - 관리자용
	public int setWorkToolPgUpdate(NewMtrltoolMgtDto toolMgtDto) {
		int returnKey = -1;
		try {
			//공구 사용이력 - 수정
			newMtrltoolMgtMapper.updateToolUse(toolMgtDto);
	...
	public int setWorkToolPgDelete(List<NewMtrltoolMgtDto> toolMgtList) {
		...
		try {
			for(NewMtrltoolMgtDto toolMgtDto : toolMgtList) {
				//공구 사용이력 - 삭제
				returnKey = newMtrltoolMgtMapper.deleteToolUse(toolMgtDto);
