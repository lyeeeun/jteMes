...
window.onload = function () {
	...
	canvas.addEventListener("click", function (e) {
		...
		if(window_x > 1900) {
			if((window_x*0.79>e.layerX && e.layerX>window_x*0.74)&&(window_y*0.70>e.layerY && e.layerY>window_y*0.68)) {
				page_move(dateCombine("eng_Search"),"low_voltage","LowVoltage","/low_voltage");
			}
		} else if(window_x > 1700) {
			if(window_x*0.78>e.layerX && e.layerX>window_x*0.73&&(window_y*0.69>e.layerY && e.layerY>window_y*0.67)) {
				page_move(dateCombine("eng_Search"),"low_voltage","LowVoltage","/low_voltage");
			}
		} else if(window_x > 1500) {
			if(window_x*0.77>e.layerX && e.layerX>window_x*0.72&&(window_y*0.68>e.layerY && e.layerY>window_y*0.66)) {
				page_move(dateCombine("eng_Search"),"low_voltage","LowVoltage","/low_voltage");
			}
		}
	});
...
function top_line(context) {
	...
	context.font = "12px malgun gothic";
	context.fillText("#1 STEP UP TR",795,100);
...
function middle_line_common(context) {
	...
	context.font = "12px malgun gothic";
	context.fillStyle = "black";
	context.fillText("VCB-AU1",1535,390);
	...
	context.font = "12px malgun gothic";
	context.fillText("#1 AUX. TR",1520,600);
	...
	context.lineTo(1700, 820);
	
	context.lineTo(1700, 830);
	context.lineTo(1790, 830);
	context.lineTo(1810, 820);
	context.lineTo(1790, 810);
	context.lineTo(1700, 810);
	context.lineTo(1700, 820);
	
	context.font = "10px malgun gothic"
	context.fillStyle = "black";
	context.fillText("480V SWITCHYARD",1705,823);
...
function outgoing(context, json, x, y) {
	...
	if(json == "no_data") {
		...
		data[0][0] = json.CA0001; 
		data[1][0] = json.CA0004; 
		data[2][0] = json.CA0002; 
		data[3][0] = json.CA0005; 
		data[4][0] = json.CA0003; 
		data[5][0] = json.CA0006; 
		data[6][0] = json.CA0007; 
		data[7][0] = json.CA0008; 
		data[8][0] = json.CA0009; 
		data[9][0] = json.CA0010; 
		data[10][0] = json.CA0011; 
		data[11][0] = json.CA0012; 
	}
...
function AUX(context, json, x, y) {
	...
	if(json == "no_data") {
		...
		data[0][0] = json.CA0025; 
		data[1][0] = json.CA0028; 
		data[2][0] = json.CA0026; 
		data[3][0] = json.CA0029; 
		data[4][0] = json.CA0027; 
		data[5][0] = json.CA0030; 
		data[6][0] = json.CA0031; 
		data[7][0] = json.CA0032; 
		data[8][0] = json.CA0033; 
		data[9][0] = json.CA0034; 
	}
