...
<div style="display:none;">
	<div id="pkgPlanInfo">
		...
		<!-- 포장 작업 정보 -->
			<div class="pop_title"><spring:message code = "col_rlesPlanCd"></spring:message></div>
			<table style="width:100%;" id="jtePopForm_master" class="jtePopForm_master">
				...
				<tr>
				<!-- 포장 계획 수량 -->
					<td><i class="fas fa-caret-right left-caret"></i><spring:message code = "col_releasePlanQty"></spring:message> : </td>
...
this.lfn_jteSgGrd_setGrd = function(){
	...
	*/
	...
	lfo_common.columns = [
		...
		{field: "itemPackageStock",  title: gfn_getMsg("col_releaseQty"), width: "100px"},			//포장수량
		{field: "packageWork", title: gfn_getMsg("col_releasePlan"), width:"100px",				//포장계획
			template : function(rows) {
				return '<button class="k-button k-primary" type="button" onclick="lfn_packagePlan(this)"><spring:message code = "mn_rlesPlanMgt"></spring:message></button>';
			}
		},
	];
...
this.lfn_jtePopGrd_popOpen = function(mode, selectedItem){
	var options = {
		...
		height: "470px", 
		id:lfo_common.popId, 
		title: gfn_getMsg("col_releasePlan"),			//포장작업
		resizable:true,
...
this.lfn_jtePopGrd_setGrd = function(selectedItem){
	...
	//3.
	lfo_popGrd.ctrlUrl = "/cform/ship/rlesPlanMgt";
	...
	lfo_popGrd.columns = [
		...
		{field: "pkgOption", template:'<button class="k-button k-primary" type="button" onclick = "lfn_pkgPlanPop_open(\'EDT\',this); " style="min-width:60px;">' + gfn_getMsg("btn_update") + '</button>',
			title:gfn_getMsg("btn_update"), width: "120px;"}, 										//수정
		{field: "pkgId", title:gfn_getMsg("col_rlesPlanCd"), width: "170px;"},						//포장계획코드
		...
		{field: "pkgState", template: "# var item = gfn_isNull(gfn_getCode(pkgState))== true ? '기초코드 없음' : gfn_getMsg('bc_'+ gfn_getCode(pkgState).cdId , gfn_getCode(pkgState).cdNm) # #= item #",
			title:gfn_getMsg("col_releaseState"), width: "120px;"},								//포장 진행 상태
...
this.lfn_pkgPlanPop_open = function(mode, selectedItem){
	var options = {
		modal:true, 
		width: "750px",
		height: "280px",  /* 200507 출고 관리 그리드의 포장 작업 버튼 클릭 → 추가 버튼 클릭 시의 팝업창 height 변경_550->360px LYM 200511 JJW 550-> 410px -> 280 */
		id : "pkgPlanInfo", 
		title:gfn_getMsg("col_rlesPlanRegist"),			//포장 작업 등록
		resizable:true,
		animation:{open :{effects:"expand:vertical fadeIn"},
		close :{effects:"expand:vertical fadeIn", reverse: true}},
		actions:[/*"Minimize", "Maximize", */"Close"]
 	...
 	$.ajax({
		async:false,
		url : "/cform/ship/rlesPlanMgt/getPackageAgg",
		type: "POST",
