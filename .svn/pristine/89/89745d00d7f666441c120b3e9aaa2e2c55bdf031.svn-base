...
this.lfn_jteDbGrd_setGrd = function(){
	...
	lfo_dateBox.dayArray.forEach(function(item){
		...
		dayDataCol.columns.push({
			title: item, field: "planQty"+item, format: "{0:n0}", width: "87px",
	...
	lfo_common.columns = [
		...
		{field: "itemStdStr01", title : gfn_getMsg("col_itemStandard"), width: "120px", locked: true},											//예비1(Str)
		{field: "itemUnit", template:"# var item = gfn_isNull(itemUnit)== true ? '' : gfn_getMsg('bc_'+ gfn_getCode(itemUnit).cdId , gfn_getCode(itemUnit).cdNm) # #= item #",
				title:gfn_getMsg("col_itemUnit"), width: "80px", locked: true},
		{field: "planTotalQty", format: "{0:n0}", title:gfn_getMsg("col_totalPlanQuan"), width: "120px", locked: true},				//총계획량
	...
	lfo_common.selectBox = [
		{ text: gfn_getMsg("col_itemNm"), value: "itemCal.itemNm" },
		{ text: gfn_getMsg("col_itemCd"), value: "itemCal.itemIId" },
	];
...
this.lfn_jtePop_open = function(selectedItem, selectedDate){
	var options = {
			...
			width: "400px",
			height: "250px", /** 20.08.28 JJW 230->250 **/
			workCd : "", 
			bdSeq : "",
			resizable : true,
