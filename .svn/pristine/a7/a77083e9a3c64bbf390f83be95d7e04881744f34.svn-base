		...
		<form id="jtePopForm_form" class="itemMgtForm">
			<table id="jtePopForm_master" class="jtePopForm_master">
				...
				<!-- 부품명 -->
					...
					<td colspan="3"><input type="text" id="itemNm" class="center-input" name="itemNm" format="text"  /></td>
				</tr>
				<tr>
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_partSpecification"></spring:message> :</td>
					<td colspan="3"><input type="text" id="itemStdStr01" class=center-input name="itemStdStr01"/></td>
				</tr>
				<tr>
				<!-- 금액 -->
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_cost"></spring:message> :</td>
					<td><input type="text" id="itemPrice" class="left-input" name="itemPrice" format="currency"/></td>
				<!-- 무게 -->
...
<%--				<tr>
				...
				<!-- 단위 -->
					...
					<td><input type="text" id="itemUnit" class="left-input" name="itemUnit" format="selBox" msg="comn_unit" /></td>
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_productionLine"></spring:message> :</td>
					<td>
						<input id="itemStdStr02" class="right-input" name="itemStdStr02" format = "selBox" custom="true" keyGbn="placeId" keyText="placeNm" func="lfn_placeParent_get()" />
<!-- 						<input type="text" id="placeNm" class="left-input" name="placeNm" readonly/> -->
<!-- 						<input type="hidden" id="itemStdStr02" name="itemStdStr02"/> -->
<!-- 						<button class="k-button k-primary search-btn" type="button" onclick="lfn_placePlacePop_open(lfn_placePop_callback);" >조회</button> -->
<!-- 						<input type="text" id="itemStdStr02" class="left-input" name="itemStdStr02" format="selBox" msg="eqmt_div" custom="true" /> -->
					</td>
				</tr>
...
this.lfn_jteSgGrd_setGrd = function(){
	...
	lfo_common.columns = [
		...
		{field: "itemStdStr01", title : gfn_getMsg("col_partSpecification"), width: "240px"},											//예비1(Str)
		{field: "placeNm", title :gfn_getMsg("col_productionLine"), width: "100px"},													//부품유형
		...
		{field: "description", title:gfn_getMsg("col_desc"), width: "240px"},									//설명
	...
	lfo_common.validation ={
		...
		itemStdStr01:{
			messages : gfn_getMsg("pop_petps"),
			rules : function(input){
				if(input.is("[name=itemStdStr01]")){
					return input.val().length != 0 && input.val().trim().length != "";
				}
				return true;
			}
...
this.lfn_jtePop_open = function(mode, selectedItem){
	...
	var options = {
			modal:true, 
			width: "700px", 
			height: "480px", /* 20.09.01 | ymlee | 530px : 신명  // 430px : 하나 201022 JJW 430-> 480*/
			id:"jtePopForm", 
...
this.lfn_companyInfo = function(mode) {
	var options = {
			modal:true, 
			width: "600px", 
			height: "470px",  /* 200409 관련 회사 관리 팝업창 높이 545px에서 수정_LYM 200810 youmi 450->470px */
			id:"company_info", 
...
//생산라인 등록 팝업
this.lfn_placePlacePop_open = function(callback){
	
	//검색조건
  	var searchPrmt ={
 		placeStrStd01 : "ParentWorkLine",
 		isDel:false
  	}
	//공통 자재관리 조회 팝업 호출
	gfn_openCustomPop('place', callback, searchPrmt);
}
//위치정보 팝업 콜백
this.lfn_placePop_callback = function(callbackObj){
	$("[name='itemStdStr02']").val(callbackObj.placeId);
