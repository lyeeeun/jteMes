...
@Service
public class ZinixUserInfoService extends CustomQueryDslSupport {

	public ZinixUserInfoService() {
		super(UserInfoVo.class);
	}
	
	@Resource
	ZinixUserInfoMapper zinixUserMapper;
	...
	public PageInfo<ZinixUserInfoDto> getUserList(ZinixUserInfoDto userInfo, PageRequestVo pageRequestVo){
		List<ZinixUserInfoDto> userList = null;
		int rowCount = 0;
		...
		try {
			//Count
			rowCount = zinixUserMapper.selectUserListCount(userInfo);
			if (rowCount > 0) {
				//List
				userList = zinixUserMapper.selectUserList(userInfo);
			}
		} catch (Exception e) {
			throw new BusinessException("수주조회 에러입니다. : " + e.getMessage());
		}
		if (userList == null) {
			userList = new ArrayList<ZinixUserInfoDto>();
		}
		return new PageInfo<ZinixUserInfoDto>(userList, pageRequestVo, rowCount);
	}
	
	
	public List<ZinixUserAuthDto> getUserRoleList(ZinixUserAuthDto userAuthDto){
		List<ZinixUserAuthDto> userAuthList = null;
		try {
			userAuthList = zinixUserMapper.selectUserRoleList(userAuthDto);
		} catch (Exception e) {
			throw new BusinessException("수주조회 에러입니다. : " + e.getMessage());
		}
		if (userAuthList == null) {
			userAuthList = new ArrayList<ZinixUserAuthDto>();
		}
		return userAuthList;
	}
	...
	@Transactional
	public int setUserSave(ZinixUserInfoDto userInfoDto){
		int resultInt = -1;
		try {
			...
			if(userInfoDto.getAction().equals("C")) {//사용자 등록
				...
				//사용자입력
				resultInt = zinixUserMapper.insertUserInfo(userInfoDto);
				//권한입력
				if(resultInt > 0)
					resultInt = zinixUserMapper.insertUserRole(userInfoDto);
			}else if(userInfoDto.getAction().equals("U")){//사용자 변경
				//사용자수정
				resultInt = zinixUserMapper.updateUserInfo(userInfoDto);
				//권한삭제
				if(resultInt > 0)
					resultInt = zinixUserMapper.deleteUserRole(userInfoDto);
				//권한입력
				if(resultInt >= 0)
					resultInt = zinixUserMapper.insertUserRole(userInfoDto);
