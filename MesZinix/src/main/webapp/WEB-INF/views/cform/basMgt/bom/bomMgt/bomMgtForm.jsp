	...
	<div id="jtePopForm">
		<form id="jtePopForm_form" class="bomMgtForm">
			<table id="jtePopForm_master" class="BasicTable">
				...
				<tr>
				<!-- BOM 코드 -->
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_bomCd"></spring:message> : </td>
					<td><input type="text" id="bomId" name="bomId" placeholder="<spring:message code = "pop_autoInput"></spring:message>" readonly/></td>													<!-- 자동입력 -->
...
this.lfn_setBomTree = function(mode, selectedItem) {
	...
	lfo_bom.ctrlUrl = "/cform/basMgt/bom/bomMgt";
	lfo_bom.crud  = {
	...
	lfo_bom.mtTreeExpand =[];
	
// 	$("#jteBomTree").on("dblclick",".k-in", function(e){
// 		var item = lfo_bom.tree.dataItem($(e.target).closest("li"));
// 	});
...
this.lfn_jtePop_open = function(mode, selectedItem){
	var options = {
		modal:true, 
		width: "700px", 
		height: "405px", /* 200413 첫번째 추가 팝업  height 수정_LYM 380 , 0428 JJW 380 -> 350 -> 300 , 201113 YUMI 300->376 */ 
		id:"jtePopForm", 
...
this.lfn_jtePopGrd_setForm = function(mode, selectedItem){
	...
	if(mode=='NEW'){
		...
		//BOM 구분에서 품목은 선택할수 없다.
		var ddl =  $("#bomTarget").data("kendoDropDownList");
...
this.lfn_bomTarget_get = function(){
	...
	bomTargetData.forEach(function(item, index){
		...
		if(item.cdId == "prcs_bom01"){
			bomTargetData.pop(index);
		}
	})
	return bomTargetData;
}
