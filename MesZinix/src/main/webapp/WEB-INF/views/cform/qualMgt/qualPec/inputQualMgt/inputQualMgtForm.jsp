	...
	<form id="jtePopForm_form" class="inputQualMgtForm" style="display:none;">
		<!-- 발주주문서 -->
			...
			<colgroup> <!-- 13 27 13 27 -->
				...
				<!--발주상태 -->
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_mtrlOrderStatus"></spring:message> : </td>
				...
				<!-- 발주일 -->
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_mtrlOrderDate"></spring:message> : </td>
					<td>
		...
		<div id="jtePopForm_grid" class="inputQualMgtForm" style="height:480px"></div>
	</div>
</div>
	...
	<form id="jteMtrlOrdChkPop_form" class="inputQualMgtForm" style="display:none;">
		<!-- 수령정보 -->
			...
			<colgroup> <!-- 15 25 15 25 완료정보 조회 버튼 때문에 1행 추가-->
				...
				<!-- 발주상황구분 -->
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_orderSitDiv"></spring:message> : </td>
				...
				<!-- 배송비용 -->
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_deliveryCost"></spring:message> : </td>
...
<div style="display:none;">
	<div id="jteMtrlBadChkPop">
		...
		<!-- 불량품 정보 -->
			...
			<table style="width:100%;" id="jteMtrlBadChkPop_master" class="BasicTable">
			<colgroup>
				<col style="width: 19%;">
				<col style="width: 31%;">
				<col style="width: 19%;">
				...
				<!-- 불량정보 -->
					...
					<td><input id="badCode" name="badCode"  format = "selBox" msg="qual_MTRLCUT"/></td> <!-- style="width: 95%; max-width:95%;" -->
				<!-- 수량 -->
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_quantity"></spring:message> : </td>
				...
				<!-- 검사자 -->
					...
					<td>
						<input id="chkUserNm"class="btn-input"  name="chkUserNm" format="text" readonly/>
						<input type="hidden" name="chkUser" id="chkUser"/><!-- 조회 -->
				...
				<!-- 검사일 -->
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_inspectDate"></spring:message> : </td>
...
this.lfn_jteSgGrd_setGrd = function(){
	...
	*/
	...
	//4.
	lfo_common.crud  = {
		read:{url:"/getMtrlOrderInputList", auth:"", prmt:{}}, 
...
this.lfn_mtrlCurPop_open = function(mode, selectedItem){
	var options = {
		modal:true, 
		width: "1000px", // 200512 JJW 그리드 더블클릭 팝업 (발주주문서) 너비 700px
		height: "524px", /* 201216 JJW 580 -> 625 */
		id : lfo_common.popId, 
		title: gfn_getMsg("pop_orderingList"),			// 발주목록
		resizable:true,
...
*/
	...
	//3.
	lfo_popGrd.ctrlUrl = "/cform/qualMgt/qualPec/inputQualMgt"
	
	//4.
	lfo_popGrd.crud  = {
		read:{url:"/getMtrlOrderPlanInputList", auth:"", prmt:{mtrlOfId:lfo_common.gridSelected.mtrlOfId}}, 
		//create:{url:"/setMtrlOrderPlanInputSave", auth:"", openFunc:"", callback : lfn_mtrlOrderCheckPop_save}, 
		update:{url:"/setMtrlOrderPlanInputSave", auth:"", openFunc:"", callback : lfn_mtrlOrderCheckPop_save}, 
	...
	lfo_popGrd.columns = [
		{selectable : true, width:"50px"},
		{field: "mtrlOrderState", editable: function(){return false;}, title:"입고 검사" , width:"110px",			//수령
			template : function(rows) {
				if (rows.mtrlOrderState == "mtrl_order_end"){
					return '<button class="k-button k-primary" type="button" onclick = "lfn_mtrlOrderCheckPop_open(this,\'EDT\') " style="width:80px;">입고 검사</button>';					//내역보기
				}else{
					return "";
				}
			}
		},
		{field: "mtrlQualSta", title: "검사완료", width: "110px",
			template : function(rows){
				
				if(rows.mtrlQualSta == 'inspecComplete'){
					return '-';
				}else{
					return '<button class="k-button k-primary" type="button" onclick = "lfn_qual_input(this); " style="min-width:70px;">' + "완료" + '</button>';
				}
			}
		},
...
this.lfn_qual_input = function(target){
	gfn_conBox({msg:"검사완료 처리하시겠습니까?", yes : function(){
		var returnTemp = lfo_popGrd.grid.dataItem($(target).closest('tr'))
		//returnTemp.del = true;
		returnTemp.action = "U";
		$.ajax({
			async:false,
...
this.lfn_mtrlBadGrd_setGrd = function(){
	...
	*/
	...
	lfo_mtrlBadGrd.columns = [
		...
		{field: "badQty",format: "{0:n0}", attributes : { style : "text-align : right;"}, title : gfn_getMsg("col_badProdQuan"), width: "90px"},						//불량량
		...
		{field: "badTargetCode", title : "발주계획코드", width: "180px"},														//불량코드
...
this.lfn_mtrlBadGrd_open = function(mode, selectedItem){
	var options = {
		modal:true, 
		width: "690px", // 200512 JJW 불량품 관리 팝업 너비 500px -> 600px
		height: "342px", // 200512 JJW 불량품 관리 팝업 높이
