...
this.lfn_jteSgGrd_setGrd = function(){
	...
	var dataSource =  new kendo.data.DataSource({
		transport: {
			read: {
				url : lfo_common.ctrlUrl+"/getDeptList",
				dataType : "json",
				traditional :true
			},create : {
				url : lfo_common.ctrlUrl + "/setDeptSave",
				dataType : "json",
				complete: function(e) {
				...
				fields: {
					deptId: { type: "string",
						validation: {
							...
							rules: function (input) {
								if (input.is("[name='deptId']")) {
								input.attr("부서코드를 입력하세요");
					...
					deptNm: { type: "string",
						validation: {
							required: true,
							rules: function (input) {
								if (input.is("[name='deptNm']")) {
								input.attr("부서명을 입력하세요");
								return input.val().length != 0 && input.val().trim().length != "";
								}
							return true;
							}
					...
					deptSeq: { type: "int",
						validation: {
							required: true,
							rules: function (input) {
								if (input.is("[name='deptSeq']")) {
								input.attr("우선순위를 입력하세요");
								return input.val().length != 0 && input.val().trim().length != "";
								}
							return true;
							}
	...
	$("#jteSingleGrid").kendoGrid({
		...
		columns: [
			...
			{field: "deptId", editable: lfn_jteSgGrd_isEditable, title:gfn_getMsg("col_deptCd"), width: "150px"},				//부서코드
