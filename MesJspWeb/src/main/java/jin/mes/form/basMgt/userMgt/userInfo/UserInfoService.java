package jin.mes.form.basMgt.userMgt.userInfo;
...
import jin.mes.mapper.basMgt.userMgt.userInfo.UserInfoMapper;
...
@Service
public class UserInfoService extends CustomQueryDslSupport {
	...
	public PageInfo<UserInfoDto> getUserList(UserInfoDto userInfo, PageRequestVo pageRequestVo){
		...
		try {
			//Count
			rowCount = userMapper.selectUserListCount(userInfo);
			if (rowCount > 0) {
				//List
				userList = userMapper.selectUserList(userInfo);
			}
		} catch (Exception e) {
			throw new BusinessException("수주조회 에러입니다. : " + e.getMessage());
		}
		if (userList == null) {
			userList = new ArrayList<UserInfoDto>();
		}
		return new PageInfo<UserInfoDto>(userList, pageRequestVo, rowCount);
	}
	
	
	public List<UserAuthDto> getUserRoleList(UserAuthDto userAuthDto){
		List<UserAuthDto> userAuthList = null;
		try {
			userAuthList = userMapper.selectUserRoleList(userAuthDto);
		} catch (Exception e) {
			throw new BusinessException("수주조회 에러입니다. : " + e.getMessage());
		}
		if (userAuthList == null) {
			userAuthList = new ArrayList<UserAuthDto>();
		}
		return userAuthList;
	}
	...
	@Transactional
	public int setUserSave(UserInfoDto userInfoDto){
		int resultInt = -1;
		try {
			...
			if(userInfoDto.getAction().equals("C")) {//사용자 등록
				//초기 패스워드 세팅
				userInfoDto.setPassword(passwordEncoder.encode(CodeUtil.getCode("pw_set",PW_DEFAULT).getCdVal()));
				
				//사용자입력
				resultInt = userMapper.insertUserInfo(userInfoDto);
				//권한입력
				if(resultInt > 0)
					resultInt = userMapper.insertUserRole(userInfoDto);
			}else if(userInfoDto.getAction().equals("U")){//사용자 변경
				//사용자수정
				resultInt = userMapper.updateUserInfo(userInfoDto);
				//권한삭제
				if(resultInt > 0)
					resultInt = userMapper.deleteUserRole(userInfoDto);
				//권한입력
				if(resultInt >= 0)
					resultInt = userMapper.insertUserRole(userInfoDto);
				...
				if(userInfoDto.getPassword() == null || userInfoDto.getPassword().equals("")) {
					CodeUtil.getCode("pw_set",PW_DEFAULT).getCdVal();
				}else {
