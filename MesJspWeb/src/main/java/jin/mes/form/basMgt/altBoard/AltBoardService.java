...
import jin.mes.mapper.basMgt.altBoard.AltBoardMapper;
...
//				++newFileCnt;
				...
				String uploadExtAllowedPattern = CodeUtil.getCode(FILE_UPLOAD_ALOW_EXT_PATTERN_SYS_FLAG_KEY, this.uploadExtAllowedPattern).getCdVal().toUpperCase();
...
//				newFilePaths+=","+targetFile.substring(this.nfsRootPath.length());
	...
	private boolean checkOverflowPop(AltBoardDto altBoardDto) {
		...
		if(altBoardDto!=null && StringUtils.isNotEmpty(altBoardDto.getBdDivCd()) && altBoardDto.getBdUseYn().equalsIgnoreCase("Y") && altBoardDto.getBdDivCd().equals("POP")) {
			...
			int maxPopBoardLimit = Integer.parseInt(CodeUtil.getCode(MAX_POP_BOARD_LIMIT_KEY, MAX_POP_BOARD_LIMIT_DEF).getCdVal());
