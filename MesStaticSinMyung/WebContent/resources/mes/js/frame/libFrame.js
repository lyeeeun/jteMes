	...
	if(typeof data == "string"){
		...
		if(gfn_isNull(contents.html())){
			if(gfn_isNull(libFrame._popUpLayerObj[menu+"_"+data.id])){
				...
				$("#"+targetId).append("<div id='"+data.id+"' style='overflow: hidden; padding: 0;'>"+libFrame._popUpLayerObj[menu+"_"+data.id]+"</div>");
...
//		}
		...
		document.getElementById(targetId).innerHTML = '<div class="k-list-container k-popup k-group k-reset" id="'+jsNm+'" data-role="popup">\n\t' + menuObj.htmlData + '\n</div>';
//		$("#_commonPopup").append("<div id='"+targetDivId+"' style='overflow: hidden; padding: 0;'></div>");
...
//	var callback = typeof callbackNdata == "function" ? callbackNdata : dataNcallback ;
	...
	var jsNm = svcUrl.replace(/\//,"").replace('..','_').replace(/(\/[a-z_])/g, function(arg){return arg.toUpperCase().replace('/','');});
