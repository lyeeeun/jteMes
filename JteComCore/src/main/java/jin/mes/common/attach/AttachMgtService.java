...
public class AttachMgtService {
	...
	@Value("#{biz['web.attach.path']?:'D://'}") // 첨부파일 경로
	...
	//첨부파일 조회(page)
	public PageInfo<AttachMgtDto> getAttachFilesPage(AttachMgtDto attachMgtDto, PageRequestVo pageRequestVo) {
		List<AttachMgtDto> attachMgtList = null;
		int rowCount = 0;

		attachMgtDto.setFirstIndex(pageRequestVo.getBlockFirstPage());
		attachMgtDto.setLastIndex(pageRequestVo.getBlockLastPage());

		try {
			rowCount = attachMgtMapper.selectAttchMgtCount(attachMgtDto);
			if (rowCount > 0) {
				attachMgtList = attachMgtMapper.selectAttchMgtPage(attachMgtDto);
			}
		} catch (Exception e) {
			throw new BusinessException("첨부파일page 조회 에러(Service) : " + e.getMessage());
		}
		if (attachMgtList == null) {
			attachMgtList = new ArrayList<AttachMgtDto>();
		}
		return new PageInfo<AttachMgtDto>(attachMgtList, pageRequestVo, rowCount);
	}
	...
	public HashMap<String, List> attachFilesTempSave(MultipartHttpServletRequest attachfiles) {
		...
		try {
			...
			String basePath = new StringBuffer().append(attachLocation).append("FRONTSTORAGE").append(new SimpleDateFormat("//yyyy-MM-dd").format(new Date())).append("/").toString();
	...
	public List<AttachMgtDto> attachFilesSave(List<AttachMgtDto> attachMgtList, String attachTarget, String attachTargetCode){
		...
		try {
			...
			//BACKSTORAGE 경로
			String backPath = new StringBuffer().append(attachLocation).append("BACKSTORAGE").append("//"+attachTarget).append(new SimpleDateFormat("//yyyy-MM-dd").format(new Date())).append("/").toString();
			backPath = backPath.replace("file:///","");
			
			String mesPath = new StringBuffer().append("/attach/").append("BACKSTORAGE").append("/"+attachTarget).append(new SimpleDateFormat("/yyyy-MM-dd").format(new Date())).append("/").toString();
			for(AttachMgtDto attachMgtDto : attachMgtList) {
				if(attachMgtDto.getAttachStatus().equals("NEW")) {
					...
					//파일경로 지정(MES)
					attachFile.setAttachLocation(mesPath);
