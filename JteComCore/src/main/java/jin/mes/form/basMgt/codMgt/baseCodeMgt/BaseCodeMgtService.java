...
public class BaseCodeMgtService {
	 ...
	 */
	@Transactional
	public String codeAdd(BaseCodeMgtDto baseCodeMgtDto){
		String returnKey = "";
		try {
			baseCodeMgtDto.setCreatorId(JteUtils.getUserId());
			baseCodeMgtDto.setUpdatorId(JteUtils.getUserId());
			baseCodeMgtMapper.codeAdd(baseCodeMgtDto);
			returnKey = baseCodeMgtDto.getCdId();
			//다국어 저장
			if(baseCodeMgtDto.getMsgList() != null && baseCodeMgtDto.getMsgList().size() > 0) {
				msgService.pgSaveMsg(baseCodeMgtDto.getMsgList(), "bc", baseCodeMgtDto.getCdId());
			}
		}
		catch(Exception ex){
			returnKey = "";
	 ...
	 */
	@Transactional
	public String codeUpdate(BaseCodeMgtDto baseCodeMgtDto){
		String returnKey = "";
		try {
			baseCodeMgtDto.setUpdatorId(JteUtils.getUserId());
			baseCodeMgtMapper.codeUpdate(baseCodeMgtDto);
			returnKey = baseCodeMgtDto.getCdId();
			//다국어 저장
			if(baseCodeMgtDto.getMsgList() != null && baseCodeMgtDto.getMsgList().size() > 0) {
				msgService.pgSaveMsg(baseCodeMgtDto.getMsgList(), "bc", returnKey);
			}
		}
		catch(Exception ex){
			returnKey = "";
