...
public class CompMgtService {
	...
	//업체 팝업 조회
	public CompMgtDto getCompPop(CompMgtDto compMgtDto){
		CompMgtDto compDto = null;

		try {
			compDto = compMgtMapper.selectCompPop(compMgtDto);
			
			//해당 수주에 포함된 Lot 정보 조회
			if(!compDto.getCompId().equals(null)) {
				CompAddrDto addrDto = new CompAddrDto();
				addrDto.setCompId(compDto.getCompId());
				compDto.setChildAddrList(compMgtMapper.selectCompChild(addrDto));
			}
		} catch (Exception e) {
			throw new BusinessException("업체정보 조회 에러입니다. : " + e.getMessage());
		}
	...
	//업체 상세주소 저장
	@Transactional
	public String setCompChildSave(CompMgtDto compMgtDto){
		String resultKey = "";
		try {
			for(CompAddrDto compAddrDto : compMgtDto.getChildAddrList()) {
