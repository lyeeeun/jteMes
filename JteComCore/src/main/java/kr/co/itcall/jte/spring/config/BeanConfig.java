...
//		cookieLocaleResolver.setDefaultLocale(Locale.ENGLISH);
	...
	public MessageSource messageSource() {
		...
		messageSource.setCacheSeconds(Integer.parseInt(CacheUtil.getCode("cacheRefreshTime", SYS_FLAG_REFLASH_SECONDS).getCdVal())); // 10분(10*60) // reload messages every 10 seconds
	...
	{
		//null인경우 에러  예외처리 (jkkim 20200917)
		BaseCodeMgtDto oneDay = CacheUtil.getCode("scheduler.oneDay");
		return oneDay == null ? "" : oneDay.getCdVal();
	}
	
	@Bean(name="schMinMtrlStock")
	public String schMinMtrlStock()
	{
		/*** Core 작업 시, Alarm Service 각 Project 별로 분리 할 것. Alarm Id Sequence로 변경 할 것. ***/
		BaseCodeMgtDto oneDay = CacheUtil.getCode("scheduler.minMtrlStock","0 5 8 * * *");
		return oneDay == null ? "" : oneDay.getCdVal();
	}
