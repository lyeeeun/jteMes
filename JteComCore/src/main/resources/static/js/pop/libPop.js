...
this.gfn_openCustomPop = function(type, callback, prmtObj){
	...
	case "userInfo" : //사용자
		...
		comPopObj.url = "/form/basMgt/userMgt/userInfo/getUserList";
		comPopObj.model = {
			id: "userId",
			fields: {
				userId: { type: "string" },
	...
	case "processInfo": //공정 정보
		...
		comPopObj.url = "/form/basMgt/operMgt/processInfo/getProcessInfo";
...
*/		];
	...
	case "eqmtMgt"://설비 마스터 정보
		...
		comPopObj.url = "/form/basMgt/operMgt/facilMgt/getEqmtMgtList";
		comPopObj.prmtNm = ["use"];
		comPopObj.prmtVal = [true];
		comPopObj.model = {
				id: "eqmtMgtId",
	...
	case "tool"://공구 마스터 정보
		...
		comPopObj.url = "/form/basMgt/operMgt/toolMgt/getToolInfoList";
		comPopObj.prmtNm = ["use"];
		comPopObj.prmtVal = [true];
		comPopObj.model = {
				id: "toolId",
		...
		comPopObj.columns =[
			...
			//{field: "toolPrice",format: "{0:n0}", attributes : { style : "text-align : right;"}, title:"공구 단가", width: 100},
			{field: "toolLimit", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_useLimit"), width: 110},			//사용한도
...
*/			{field: "itemStdStr01", title : "부품규격", width: "100px"},										//부품규격
...
//			{field: "mtrlUseday", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:"시효기간", width: "100px"},
...
//				title:gfn_getMsg("col_unit"), width: "110px"},													//단위
			{field: "mtrlStdStr01", title:"자재규격", width: "240px"},
	 		/*{field: "itemStdStr02", title : "예비2(Str)", width: "100px"},										//예비2(Str)
	 		{field: "itemStdStr03", title : "예비3(Str)", width: "100px"},										//예비3(Str)
	 		{field: "itemStdStr04", title : "예비4(Str)", width: "100px"},										//예비4(Str)
	 		{field: "itemStdStr05", title : "예비5(Str)", width: "100px"},										//예비5(Str) */
			//{field: "mtrlCost", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:"단가", width: 100},
			{field: "mtrlDesc", title:gfn_getMsg("col_desc"), width: "250px"}									//설명
	...
	case "materialMgt"://자재상세
		...
		comPopObj.url = "/form/mtrl/mtrlMgt/getMtrlMgtList";
		comPopObj.prmtNm = ["use"];
		comPopObj.prmtVal = [true];
		comPopObj.model = {
			id: "mtrlMgtId",
		...
		comPopObj.columns =[
			...
			{field: "mtrlStrStd01", title:"자재규격", width: 200},
...
*/			{field: "mtrlStd03", format: "{0:n}", title:gfn_getMsg("col_weight"), width: 90},								//무게
			...
			{field: "mtrlMgtCnt", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_mtrlQty"), width: 110},							//자재수량
	...
	case "mtrlOrderForm": // 발주주문서정보
		comPopObj.type = type;
		comPopObj.callback = callback;
		comPopObj.width = 800;
		comPopObj.height = 315;
		comPopObj.title = "발주 주문서 정보";			
		comPopObj.prmtNm = [];
		comPopObj.prmtVal = [];
		comPopObj.url = "/form/mtrl/mtrlOdMgt/mtrlOdcursitMgt/getMtrlOrderFormList";
		comPopObj.model = {
				fields: {
	...
	case "company"://공급+고객 검색
		...
		}else if (type == "makerorg"){ //매출처만
			comPopObj.prmtNm = ["compType","use"];
			comPopObj.prmtVal = ["corp_div03",true];
		...
		comPopObj.columns = [
			...
			{field: "compStdStr02", title:gfn_getMsg("col_compStdStr02", "거래처코드"), width:140},
	...
	case "routing"://라우팅
		...
		comPopObj.url = "/form/basMgt/routing/routingMgt/getRoutingInfo";
		comPopObj.prmtNm = [];
		comPopObj.prmtVal = [];
		comPopObj.model = {
			id: "routingId",
		...
		comPopObj.columns =[
			...
			{field: "routingSeq", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_routSeq"), width: 90},				//차수
			{field: "manHour", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_processPeriod"), width: 120},			//공정시간
	...
	case "routingEquip"://라우팅 - 설비
		...
		comPopObj.url = "/form/basMgt/routing/routingMgt/getRtlEqmtList";
		comPopObj.prmtNm = [];
		comPopObj.prmtVal = [];
		comPopObj.model = {
			id: "eqmtMgtId",
	...
	case "lotInfo"://Lot 정보
		...
		comPopObj.prmtNm = [];
		comPopObj.prmtVal = [];
		comPopObj.url = "/form/orderMgt/orderSheet/orderDetailMgt/getLotList";
		comPopObj.model = {
			id: "lotSeq",
			fields: {
				lotSeq : { type: "int" },
...
*/			{field: "itemNm", title:gfn_getMsg("col_itemNm"), width: "220px;"},				//부품명
	...
	case "lotStock"://Lot별 부품재고 정보
		...
		comPopObj.prmtNm = [];
		comPopObj.prmtVal = [];
		comPopObj.url = "/form/ship/shipPlanMgt/getLotStockList";
		comPopObj.model = {
			id: "lotId",
			fields: {
				lotId : { type: "string" },
				...
				rlesMgtQty: { type: "int" },
				rlesWorkQty: { type: "int" },
				rlesPlanTotal : { type: "int" },
				rlesPlanNeed : { type: "int" },
				rlesExceptQty : { type: "int" },
				shipEndQty: { type: "int" },
				shipPlanTotal : { type: "int" },
				shipPlanNeed : { type: "int" },
				shipExceptQty : { type: "int" },
				rlesStockQty: { type: "int" }
			}
		};
		comPopObj.columns = [
			{field: "lotId", title:gfn_getMsg("col_lotCd"), width: "170px;"},				//Lot코드
			{field: "lotNm", title:gfn_getMsg("col_lotWrkNm"), width: "170px;"},				//Lot명
			{field: "itemNm", title:gfn_getMsg("col_itemNm"), width: "170px;"},				//부품명
			{field: "lotQty", title:gfn_getMsg("col_lotQty"), width: "100px;"},				//Lot수량
			{field: "itemStock", title:gfn_getMsg("col_itemStockQty"), width: "120px;"},	//부품생산수량
			{field: "rlesWorkQty", title:gfn_getMsg("col_rlesWorkQty"), width: "120px;"},	//출고작업수량
			{field: "orderId", title:gfn_getMsg("col_orderCd"), width: "120px;"},				//수주코드
			{field: "orderNm", title:gfn_getMsg("col_orderNm"), width: "120px;"},				//수주명
	...
	case "orderInfo": //수주 정보
		...
		comPopObj.prmtNm = [];
		comPopObj.prmtVal = [];
		comPopObj.url = "/form/orderMgt/orderSheet/orderDetailMgt/getOrderList";
		comPopObj.model = {
				id: "orderId",
				fields: {
					orderId: { type: "string" },
		...
		comPopObj.columns = [
			...
			{field: "orderCost", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_cost"), width: "110px"},							//비용
	...
	case "itemMgt": // 제품정보
		...
		comPopObj.width = 670;
		comPopObj.height = 315;
		comPopObj.title = gfn_getMsg("pop_itemInfo");			//출하업체 정보
		comPopObj.prmtNm = ["del"];
		comPopObj.prmtVal = [false];
		comPopObj.url = "/form/item/itemStockMgt/getItemStockList";
		comPopObj.model = {
				fields: {
					itemId : { type: "string" },
					itemNm : { type: "string" },
		...
		comPopObj.columns = [
			{field: "itemMgtId", title:gfn_getMsg("col_itemIndivCd"), width: "130px"},													
			{field: "itemNm", title:gfn_getMsg("col_itemNm"), width: "220px"},															
			{field: "itemMfgDate", format:"{0: yyyy-MM-dd hh:mm:ss}", title:gfn_getMsg("col_manufactDate"), width: "150px"},			
			{field: "itemStock", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_indivStockQty"), width: "110px"},								
			{field: "itemPackageStock", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_packQuan"), width: "110px"},							
	...
	break;
	case "Barcode": // Barcode 사용 업체 대상(출고 관리 연계)
		comPopObj.type = type;
		comPopObj.callback = callback;
		comPopObj.width = 670;
		comPopObj.height = 315;
		comPopObj.title = "Barcode 등록(수동)";			
		comPopObj.prmtNm = [];
		comPopObj.prmtVal = [];
		comPopObj.url = "/cform/ship/pickMgt/selectBarcodeInfo";
		comPopObj.model = {
				fields: {
					barcode : { type: "string" }
				}
		...
		comPopObj.shBoxItem = [
			{ text: "Barcode", value: "barcodeInfo.barcode" }
		];
	break;
	case "assignInfo": //Lot 별 작업 지시 정보
		comPopObj.type = type;
		comPopObj.callback = callback;
		comPopObj.width = 670;
		comPopObj.height = 315;
		comPopObj.title = "작업 지시 정보";			//출하업체 정보
		comPopObj.prmtNm = [];
		comPopObj.prmtVal = [];
		comPopObj.url = "/form/mfgMgt/productWork/getProdWorkAsgnList";
		comPopObj.model = {
				fields: {
					prodAsmId: { type: "string" },
					prodAsmNm: { type: "string" },
					prodAsmDate: { type: "string" },
					prodAsmQty: { type: "string" },
					prodAsmState: { type: "string" },
				}
		};
		comPopObj.columns = [
			{field: "prodAsmId", title:"작업지시 ID", width: "130px"},									
			{field: "prodAsmNm", title:"작업지시 명", width: "130px"},									
			{field: "prodAsmDate", title:"작업지시 일자", width: "130px"},									
			{field: "prodAsmQty", title:"작업지시량", width: "130px"},									
			{field: "prodAsmState", title:"작업 상태", width: "130px"}							
		];
//		comPopObj.shBoxItem = [
//			{ text: gfn_getMsg("col_compAddr"), value: "compAddr.comp_addr_detail" },
//		];
...
/***
 * 
 * Excel Migration Popup
 * - libContent.js 의 excel 허용 Option과 다름.
 * - Upload & Download 구현
 * 
 * ***/
this.gfn_excelUplodePop = function(menuId) {
	if($("#excelPopArea").length > 0){
		$("#excelPopArea").data("kendoWindow").close();
		$("#excelPopArea").remove();	
	}
	
	$("#libExcelUploadPop").append("<div id='excelPopArea' style='height:100%;'></div>");
	/* 21.01.29 JJW 엑셀 업로드 팝업 */
	$("#excelPopArea").append('<form id="excelUploadForm" name="excelUploadForm" method="post" enctype="multipart/form-data">' +
							...
							'<input type="file" id="fileInput" name="fileInput" style="border: 1px solid #a0bbf3; width: 93%; padding-top: 10px; padding-bottom: 10px; padding-left: 10px; border-radius: 6px;">' +
							'<input type="hidden" id="menuId" name="menuId">' +
							'</td>' +
							'</tr>' +
							'<tr>' +
							'<td style="text-align: center;">' +
							'<button type="button" onclick="gfn_excelUpload()" class="k-button k-primary" style="width:90%; margin-left: 0px;">Excel Upload</button>' +
							'</td>' +
							'<td style="text-align: center;">' +
							'<button type="button" onclick="gfn_excelDownload(\'' + menuId + '\')" class="k-button k-primary" style="width:90%; margin-left: 0px;">Excel양식 저장</button>' +
							'</td>' +
							'</tr>' +
							'</table>' +
							'</form>' );
	...
	$("#excelPopArea").kendoWindow({
		width: "530px", /* 21.01.29 JJW 500 -> 530 */
		height: "150px", /* 21.01.29 JJW 500 -> 150 */
		position: {
...
this.gfn_excelDownload = function(menuId) {
	...
	$.ajax({
		url: "/excelUtil/downloadExcelFile",
		...
		success: function(data){
			...
			$('#excelUploadForm')[0].reset(); // this will reset the form fields
		}
	})
}


this.gfn_excelUpload = function(){
	if($("#fileInput").val() == "") {
		kendo.confirm("파일을 선택하여 주십시오.");
...
//검사 체크리스트 팝업 
this.gfn_inspectPop = function(mode, inspectMode, popId, param, tagt) {
	
	var option = {};
	
	option.popAreaId = popId + "_Form";
	option.popAreaTableId = option.popAreaId + "_tb";
	
	// Popup Form 영역 추가
	$("#" + popId).append("<form id='" + option.popAreaId + "' class='" + option.popAreaId + "'></form>");
	
	if(inspectMode == "incommingInspect") {
		option.width = 760;
		option.height = 530;
		option.title = "입고 검사";			//발주현황 (자재정보), 입고검사
		option.baseGroupCode = "inputQualChk";
		option.category = "incomming_inspect";
	} else {
		kendo.confirm("잘못 된 검사 모드입니다.");
		return false;
	}
	
	$("#" + option.popAreaId).append("<table id='" + option.popAreaTableId + "' style='width:100%;' class='mtrlOdcursitMgtForm-table'>" +
			"<colgroup>" +
			...
			"<td>No</td>" +
			"<td>" + gfn_getMsg("col_mtrlOdcursitItem") + "</td>" +
			"<td>" + gfn_getMsg("col_mtrlOdcursitStandard") + "</td>" +
			"<td>" + gfn_getMsg("col_valueInput") + "</td>" +
			"</tr>");
	
	$.each(gfn_getCode(option.baseGroupCode), function(index, item) {
		$('#' + option.popAreaTableId).append("<tr id=" + item.cdId + ">" +
	...
	$("#" + option.popAreaId).append("</table>");
	
	$("#" + option.popAreaId).append("<div class='mtrlOdcursitMgtForm final-btn-area' style='text-align:center;'>" +
			"<button class='k-button k-primary final-btn' type='button' onclick='gfn_inspectPop_save(\"" + popId + "\", \"" + tagt + "\", \"" + option.category + "\", \"" + mode + "\")'>" + gfn_getMsg("pop_save") + "</spring:message></button>" +
			"<button class='k-button k-primary final-btn' type='button' onclick='gfn_closePop(\"" + popId + "\")'>" + gfn_getMsg("pop_cancel") + "</spring:message></button>" +
			"</div>");

	var popOption = {
		modal:true, 
		width: option.width, //200518 JJW 체크리스트 팝업 너비 900 -> 650
		height: option.height, //200518 JJW 체크리스트 팝업 높이 550 -> 400
		id : popId, 
		title:option.title,			// 체크리스트 확인
		resizable:false,
	...
	popOption.callback = function(){};
	this.gfn_winOpen(popOption);
	
	if(mode == "EDT") {
		$.ajax({
			async:false,
			url : "/form/qualMgt/qualCheckList/selectCheckList",
			type: "POST",
			data: {inspectId: param, targetId: tagt},
			traditional :true,
			success: function(data){
...
this.gfn_inspectPop_save = function(popId, tagt, category, mode){

	var insertInspectList = new Array();
	var updateInspectList = new Array();
	var svcUrl = "/form/qualMgt/qualCheckList";
	var savePrmt = {};
	
	if(mode == "NEW") {
		$("#" + popId + "_Form_tb").find('tr').each(function(index, item) {
			if(index == 0) {
				return true
			}
			insertInspectList.push({ questionId: $(item).attr('id'), questionValue: $(item).find('input').val(), targetId: tagt, category: category});
		});
		...
		$("#" + popId + "_Form_tb").find('tr').each(function(index, item) {
			...
			updateInspectList.push({ questionId: $(item).attr('id'), questionValue: $(item).find('input').val(), targetId: tagt});
		});
