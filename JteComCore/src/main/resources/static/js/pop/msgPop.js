...
this.gfn_msgSetPop = function(option){
	...
	if(msgPopObj.mode=='NEW'){
		...
		msgPopObj.height = 283; // 20.04.28 JJW 다국어관리 팝업 창 크기 수정 220 -> 260  LYM 220->285
		...
		msgPopObj.title = gfn_getMsg("pop_msgRegist");									//다국어 등록
		msgPopObj.width = 460;  // 20.04.29 JJW 다국어등록 팝업 창 크기 수정 400 -> 460  LYM 400->460  20.11.11 YUMI 260->288
		msgPopObj.height = 283;  // 20.04.29 JJW 다국어등록 팝업 창 크기 수정 220 -> 260  LYM 220->235
		msgPopObj.callback = function(){
		...
		msgPopObj.height = 283; // 20.04.28 JJW 다국어수정 팝업 창 크기 수정 220 -> 260  LYM 220->240
		msgPopObj.callback = function(){
			if(gfn_isEmpty(msgPopObj.saveMsg)){
				$.ajax({
					async:false,
					url : "/form/basMgt/codMgt/msgMgt/getMsgMergeOne",
					type: "POST",
					traditional :true,
...
this.gfn_setMsgSave = function(formID){
	...
	switch(msgPopObj.mode){
		...
		$.ajax({
			async:false,
			url : "/form/basMgt/codMgt/msgMgt/updateMsg",
			type: "POST",
