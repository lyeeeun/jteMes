			...
			activate: function( event, ui ) {
				...
				for(var index=0;index<nowTotTabCnt;++index){
					if(libTabs._tabs[index].id==ui.newPanel[0].id){
						...
						//tab 선택시 메뉴 스크롤 변경
						//document.getElementById("menuViewId_" + $("#_currentMenuId").val()).scrollIntoView();
...
libTabs._fnAddTab = function(menuInfo, data, callback){
	...
	for(var index=0;index<nowTotTabCnt;++index){
		if(libTabs._tabs[index].jsNm == menuInfo.jsNm){
			//console.log(libTabs._tabs[index].jsNm);
			// 이미 열려있다.
...
//	++libTabs._nowTabCnt; 의미 없을듯.
	...
	var tabClass = menuInfo.menuId + "Main";
		
	data.tabId = tabId;
	data.formId = menuInfo.jsNm;

	// 컨트롤탭 추가                                   menuInfo.msgViewPath             style="cursor:pointer" 포인터만 변경하는 것도 고려해볼만함. id컨트롤 안하고 onclick이벤트 줌.
	var tabMenuCtrl = '<li id="'+tabId+'-li" class="link"><span><a href="#'+tabId+'" class="txt" title="'+menuInfo.msgViewPath+'">zzz'+menuInfo.msgView+'</a>';
	...
	// 탭바디 추가
	$( "<div id='"+tabId+"' class='"+tabClass+"'><br><br><br><span><p>...["+menuInfo.msgViewPath+"] 메뉴화면을 불러오는 중입니다...</p></span></div>" ).appendTo( tabBody );
