...
<div style="display:none;">
	<div id="jtePopForm">
		<div id="jtePopForm_grid" class="jtePopForm_grid facilToolMgtForm_jtePopForm_grid"></div>
	</div>
</div>

<div style="display:none;">
	<div id="jteToolPosForm">
		<div id="jteToolPosForm_grid"></div> 
...
this.lfn_init = function(){
	dataSource = new kendo.data.HierarchicalDataSource({
		transport: {
			read: {
				url: "/form/basMgt/operMgt/rlehoMgt/getPlaceTree",
				dataType: "json"
			}
		},
		schema: {
...
// 		destroy:{url:"/setToolMgtDelete", auth:"", callback:lfn_jteGrid_delete}
	...
	lfo_common.columns = [
		...
		{field: "eqmtMgtMtnc", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_mtnc"), width: "100px"},								//정비주기
...
*/
	...
	//3.
	lfo_popGrd.ctrlUrl = "/form/facilMgt/facilToolMgt"
	...
	lfo_popGrd.columns = [
		...
		{field: "toolMgtLimit", format:"{0:n0}", attributes : { style : "text-align : right;"}, title : gfn_getMsg("col_useLimit"), width: 110},							//사용한도
		{field: "toolMgtUse", format:"{0:n0}", attributes : { style : "text-align : right;"}, title : gfn_getMsg("col_toolUseQuan"), width: 110},							//공구사용량
...
this.lfn_jteToolPos_popOpen = function(mode,selectedItem){
	var options = {
		modal:true, 
		width: "650px", 
		height: "380px", //200507 그리드 더블 클릭→설비 장착 공구 팝업의 추가 버튼 클릭 시의'장착 가능 공구 조회' 팝업 크기 변경 200811 youmi 330->345px 201111 JJW 345 -> 380px
		id : lfo_popGrd.popId, 
...
*/
	...
	lfo_toolPos.columns = [
		...
		{field: "toolMgtPurchase", format:"{0: yyyy-MM-dd}", title : gfn_getMsg("col_purchaseDate"), width: 120},			//입고일
		{field: "toolMgtVerif", format:"{0: yyyy-MM-dd}", title : gfn_getMsg("col_verifyPeriod"), width: 120},				//검증기간
		{field: "toolRpm", format:"{0:n0}", attributes : { style : "text-align : right;"}, title : gfn_getMsg("col_toolRpm"), width: 100},									//RPM
		{field: "toolCount", format:"{0:n0}", attributes : { style : "text-align : right;"}, title : gfn_getMsg("col_toolCount"), width: 100},								//카운트
		{field: "compNm", title : gfn_getMsg("col_compNm"), width: 120},													//업체명
		{field: "toolMgtDesc", title : gfn_getMsg("col_desc"), width: 300}													//설명
	];
