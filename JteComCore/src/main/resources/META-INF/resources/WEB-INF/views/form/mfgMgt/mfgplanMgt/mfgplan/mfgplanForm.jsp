...
<div style="width:100%;">
<!-- 	최상단 검색영역 -->
	<div style="width:100%; background:#f0f5ff; padding:15px 0 15px 5px; margin:10px 0 5px 0;">
		<!-- 월/일 체크박스 -->
...
this.lfn_jteSgGrd_setGrd = function(){
	...
	//3.
	lfo_common.ctrlUrl = "/form/mfgMgt/mfgplanMgt/mfgplan";
	...
	lfo_dateBox.monthArray.forEach(function(item){
		...
		monthDataCol.columns.push({
			title: item , 
			columns: [
				{field: "planQty"+item, format: "{0:n0}", attributes : { style : "text-align : right;"}, title: gfn_getMsg("col_planQuan"), attributes:{"class":"planCol","planAttr":lfo_dateBox.yearVal+"_"+item}, width: "87px"},			//계획량
				{field: "workQty"+item, format: "{0:n0}", attributes : { style : "text-align : right;"}, title: gfn_getMsg("col_prodQuan"), width: "87px"}																					//생산량
			]
		});
	});
	...
	lfo_common.columns = [
		...
		{field: "itemNm", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_itemNm"), width: "230px", locked: true},							//부품명
		...
		{field: "lotQty", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_lotQty"), width: "120px"},							//Lot수량
		{field: "planTotalQty", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_totalPlanQuan"), width: "120px"},				//총계획량
		{field: "workTotalQty", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_totalProdQuan"), width: "120px"},				//총생산량
		monthDataCol,
...
this.lfn_jteDbGrd_setGrd = function(){
	...
	lfo_dateBox.dayArray.forEach(function(item){
		...
		dayDataCol.columns.push({
			title: item , 
			columns: [
				{field: "planQty"+item, format: "{0:n0}", attributes : { style : "text-align : right;"}, title: gfn_getMsg("col_planQuan"),						//계획량
	...
	lfo_common.columns = [
		...
		{field: "itemDeduction",format: "{0:n0}", attributes : { style : "text-align : right;"} , title:gfn_getMsg("col_deduction"), width: "120px", locked: true},					//공제량
		{field: "itemDayTarget",format: "{0:n0}", attributes : { style : "text-align : right;"} , title:gfn_getMsg("col_dayTarget"), width: "120px", locked: true},					//itemDayTarget
		{field: "orderStdt", format:"{0: yyyy-MM-dd}", title:gfn_getMsg("col_orderDate"), width: "120px"},				//수주일
		{field: "orderEddt", format:"{0: yyyy-MM-dd}", title:gfn_getMsg("col_deliveryDate"), width: "120px"},			//납기일
		{field: "lotState", template:"# var item = gfn_isNull(lotState)== true ? '' : gfn_getMsg('bc_'+ gfn_getCode(lotState).cdId , gfn_getCode(lotState).cdNm) # #= item #",
				title:gfn_getMsg("col_orderStatus"), width: "120px"},										//수주상태
...
this.lfn_jtePopGrd_setGrd = function(){
	...
	//3.
	lfo_popGrd.ctrlUrl = "/form/mfgMgt/wrkinTeamMgt"
...
this.lfn_jteProdWorkGrd_setGrd = function(){
	...
	//3.
	lfo_prodWorkGrd.ctrlUrl = "/form/mfgMgt/wrkinWrkerMgt"
	2
	//4.
...
// 		{field: "deptNm", title:"부서명", width: 120},
		...
		{field: "prodWorkGood", format:"{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_prodQuan"), width: "110px"},								//양품량
		{field: "prodWorkBad", format:"{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_badProdQuan"), width: "110px"},							//불량량
