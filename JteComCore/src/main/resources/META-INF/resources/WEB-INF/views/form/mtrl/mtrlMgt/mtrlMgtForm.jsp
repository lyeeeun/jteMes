		...
		<form id="jtePopForm_form" class="mtrlMgtForm popfr-mtrlMgtForm">			
			...
			<colgroup>
				...
				<tr>
					...
					<i class="fas fa-info-circle" title="<spring:message code = "자재코드를 조회하셔야 자재개별코드가 부여됩니다."/>"></i> : </td>
					<td>
						<input type="text" id="mtrlId" class="btn-input" name="mtrlId"  placeholder="<spring:message code = "pop_searchAuto" text = "조회시 자동입력" />" readonly />			<!-- 조회시자동입력 -->
						<button class="k-button k-primary" type="button" id="mtrl_search_btn" onclick= "lfn_mtrlInfoPop_Open();" style="display:none;"><spring:message code = "btn_search" text = "조회" /></button>
					</td>
				...
				<tr>
					<td colspan="4" class="pop_detail_btn_area" style="padding-top:15px; text-align:center;" >   <!-- style="text-align:center;", 패딩 값 공통css에서 적용되므로 인라인스타일을 이용해 값 설정 -->
						<span class="pop_detail_btn pop_detail_hide" onclick="lfn_popDetail_toggle()">
						<spring:message code = "pop_expHide" text = "펼치기/숨기기" /></span>
					</td>
				</tr>
			</table>
			<div class="pop_title"><spring:message code = "pop_mtrlSpec" text = "자재특성" /></div><!--200512_편의상 div 및 테이블로 나눔_LYM -->
			<table id="jtePopForm_master-2" class="BasicTable">
			<colgroup>
				<col style="width: 17%;">
				<col style="width: 33%;">
				<col style="width: 17%;">
				...
				<tr>
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_mtrlIndivCd" text = "자재개별코드" /> : </td>	
					<td><input type="text" id="mtrlMgtId" name="mtrlMgtId" placeholder="<spring:message code = "pop_autoInput" text = "자동입력" />" readonly /></td>	
		...
		<form id = "jtePopAddForm_form" class="mtrlMgtForm">
			...
			<colgroup>  <!-- 200511 원래는 23 27 23 27_LYM -->
				...
				<tr>
					...
					<td><input id="mtrlQtyDate" name="mtrlQtyDate" format="datepicker" /></td>
				...
				<!-- 수동입력에서는 입력하지 않음-->
				<tr style="display:none;">
					<td><i class="fas fa-caret-right"></i><spring:message code = "col_progCd" text = "프로그램코드" /> : </td>
					<td><input type="text" id="mtrlQtyPgCd" name="mtrlQtyPgCd" class="k-textbox" /></td>
					<td class="td-right-title"><i class="fas fa-caret-right"></i><spring:message code = "col_progNm" text = "프로그램명" /> : </td>
					<td><input type="text" id="mtrlQtyPgNm" name="mtrlQtyPgNm" class="k-textbox" /></td>
				</tr>
				<tr style="display:hidden;">
					<td>
...
this.lfn_jteSgGrd_setGrd = function(){
	...
	//3.
	lfo_common.ctrlUrl = "/form/mtrl/mtrlMgt";
	...
	lfo_common.columns = [
		...
		{field : "mtrlMgtId", title : gfn_getMsg("col_mtrlIndivCd", "자재개별코드"), width: "150px"}, 
		{field : "mtrlMgtCnt", format: "{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_mtrlQty", "자재수량"), width: "110px"},
	...
	lfo_common.selectBox = [
		...
		{ text: gfn_getMsg("col_mtrlIndivCd", "자재개별코드"), value: "mtrlMgt.mtrl_mgt_id" },
		{ text: gfn_getMsg("col_desc", "설명"), value: "mtrlMgt.description" }
	];
...
this.lfn_jtePop_open = function(mode, selectedItem){
	...
	if(mode =="NEW"){
		options.title = gfn_getMsg("pop_mtrlRegistDetail", "자재 등록");
	}else if(mode=="EDT"){
...
this.lfn_jtePop_set = function(mode,selectedItem){
	...
	if(mode=='NEW'){
		...
		//그리드 세팅
		gfn_resizePop(lfo_common.popId, {width:750,height:580});
		$("#mtrlQtyArea").show();
...
this.lfn_jtePopGrd_setGrd = function(){
	...
	//3.
	lfo_popGrd.ctrlUrl = "/form/mtrl/mtrlMgt";
	...
	lfo_popGrd.columns = [
		...
		{field: "mtrlQtyTarget",  template:"# var item = gfn_isNull(gfn_getCode(mtrlQtyTarget))== true ? '기초코드 없음' : gfn_getMsg('bc_'+ gfn_getCode(mtrlQtyTarget).cdId , gfn_getCode(mtrlQtyTarget).cdNm) # #= item #",
			title:gfn_getMsg("col_targetDiv", "업무구분"), width: "140px"},
		{field: "mtrlQtyTargetCode", title:gfn_getMsg("col_divCd", "구분코드"), width: "180px"},
		{field: "mtrlQtyTotal", format:"{0:n0}", attributes : { style : "text-align : right;"}, title:gfn_getMsg("col_quantity", "수량"), width: "110px"},
		{field: "mtrlQtyPgCd", title:gfn_getMsg("col_progCd", "프로그램코드"), width: "180px"},
		{field: "mtrlQtyPgNm", title:gfn_getMsg("col_progNm", "프로그램명"), width: "200px"},
		{field: "mtrlQtyDate", format:"{0: yyyy-MM-dd}", title:gfn_getMsg("col_registDate", "등록일"), width: "140px"},
	...
	lfo_popGrd.selectBox = [
		...
		{ text: gfn_getMsg("col_divCd", "구분코드"), value: "u_table.mtrlQtyTargetCode" },
		{ text: gfn_getMsg("col_progCd", "프로그램코드"), value: "u_table.mtrlQtyPgCd" }
	];
	...
	$("#"+ lfo_popGrd.layoutId +" .k-checkbox").each(function(index,item){
		if($(this).attr("aria-checked") == "true"){
			...
			if(delTemp.mtrlQtyTarget != "mtrl_tagt04"){
				alert(gfn_getMsg("pop_inputNoDelete", "수동입력 외에는 삭제가 불가능 합니다."));
				return false;
			}
