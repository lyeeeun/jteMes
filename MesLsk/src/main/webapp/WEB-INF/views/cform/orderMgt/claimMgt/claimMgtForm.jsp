...
<div style="display:none;">
	<div id="jtePopForm_Claim" class="jtePopForm">  <!-- style="margin-top: 5px;" -->
		<div id="jtePopForm_Claim_gridArea" style="display:none;">
			...
			<div id="jtePopForm_Claim_grid" class="orderDetailMgtForm"></div>
		</div>
	</div>
</div>
...
<div style="display:none;">  <!-- 클레임 관리 > Lot 정보 >클레임정보 > 클레임등록 팝업창_LYM-->
	<div id="jtePopAddForm">
		<form id="jtePopAddForm_form" class="claimMgtForm claimMgtForm-popsc">
			...
			<table style="width:100%;" id="jtePopAddForm_master" class="jtePopAddForm_master claimMgtForm">
			<colgroup>
				<col class="pop-col-left-title">  <!-- style="width:15%;" -->
				<col class="pop-col-left-content">  <!-- style="width:35%;" -->	
				<col class="pop-col-right-title">  <!-- style="width:15%;" -->
				<col class="pop-col-right-content">  <!-- style="width:35%;" -->
			</colgroup>
				<tr>
				<!-- Lot코드 -->
...
<%-- 				<tr>
			...
			<table style="width:100%;" id="jtePopAddForm_master" class="jtePopAddForm_master jtePopAddForm_master_sc claimMgtForm">
			<colgroup>
...
<%-- 					<td>
				...
				<tr>
				<!-- 설명 -->
					<td><i class="fas fa-caret-right"></i><spring:message code="col_claimReason" /> : </td>
...
<div style="display:none;"> <!-- 수주 관리 그리드 더블클릭 → 팝업창 하단 그리드 더블 클릭 혹은 추가 버튼 클릭 시의 팝업창-->
	<div id="jtePopAddForm22" class="jtePopAddForm">
		<form id="jtePopAddForm_form22" class="jtePopAddForm_form orderDetailMgtForm orderDetailMgtForm_jtePopAddForm_form">
			...
			<colgroup>  <!-- 200507 원래는 col 4개에 각각의 width가 15 35 15 35였으나 col 6개로 변경_LYM -->
				...
				<tr class="pop_title">
				<!-- LoT정보 -->
					<td colspan="6"><spring:message code="col_claimSave" /></td>
				</tr>
				<tr>
				<!-- Lot코드 -->
					<td><i class="fas fa-caret-right"></i><spring:message code="col_lotCd" /> : </td>
					<td colspan="2">
						<input type="text" id="lotId" class="left-input" name="lotId" placeholder="<spring:message code = "pop_autoInput"></spring:message>"/>			<!-- 자동생성 -->
						<input type="hidden" id="lotSeq" class="left-input" name="lotSeq"/>
					</td>
				<!-- 관리자 -->
					<td class="td-right-title"><i class="fas fa-caret-right left-caret"></i><spring:message code="col_writer" /> : </td>
					<td>
						<input type="text" id="orderManagerNm" class="right-input btn-input" name="orderManagerNm" format="text" readonly/> 
						<input type="hidden" id="orderManager" class="right-input btn-input" name="orderManager"/>
					</td>
				...
				<tr>
					<td>
						<!-- Lot수량 -->
						<i class="fas fa-caret-right"></i><spring:message code="col_claimNm" /> : 
					</td>
					<td colspan="2">
						<input id="claimNm" class="left-input" name="lotQty" format="text"/>
					</td>
					
					<td class="td-right-title">
				<!-- 부품명 -->
						<i class="fas fa-caret-right left-caret"></i><spring:message code="col_quantity" /> : 
					</td>
					<td colspan="2">
				...
				<tr>
					<td><i class="fas fa-caret-right"></i><spring:message code="col_remark" /> : </td>
					<td colspan="5">
...
this.lfn_jtePopGrd_setGrd_claim = function(prmt){
	...
	lfo_claimPopGrd.columns = [
		...
		{field: "claimDesc", title:gfn_getMsg("col_claimReason"), width: "150px;"},		
	];
	//공통 그리드 세팅 호출
...
this.lfn_jtePopGrd_popOpen = function(mode,selectedItem){
	...
	var options = {
		...
		height: "410px",  ///200507 팝업창 height 변경_280->290px LYM 200810 youmi 440->450px 200820 290->302px // 201020 youmi 490->410
...
this.lfn_jtePopGrd_save = function(mode){
	...
	if(gfn_isEmpty(validation) || validation.validate()){
		...
		$.ajax({
			async:false,
			url : lfo_claimPopGrd.ctrlUrl + "/setClaimInfoSave",
			type: "POST",
...
this.lfn_claimPopGrd_delete = function(){
	...
	console.log(lfo_claimPopGrd.ctrlUrl + lfo_claimPopGrd.crud.destroy.url);
	console.log(delPrmt);
	$.ajax({
		async:false,
