...
<div style="display:none;">
	<div id="jteBadItemPop">   <!-- style="height:100%" -->
		<div class="pop_title"><spring:message code = "pop_itemBadSrh"></spring:message></div>
		<div id="jteBadItemChkPop_grid" style="height:350px;"></div>
	</div>
</div>
...
<div style="display:none;">
	<div id="jteBadItemChkPop" class="finalQualMgtForm">
		...
		<!-- 불량품 정보 -->
			<div class="pop_title"><spring:message code = "pop_itemBadInfo"></spring:message></div>
...
/* 		{field: "itemMtrl", template:"# var item = gfn_isNull(gfn_getCode(itemMtrl))== true ? '기초코드 없음' : gfn_getMsg('bc_'+ gfn_getCode(itemMtrl).cdVal , gfn_getCode(itemMtrl).cdNm) # #= item #",
		...
		{field: "itemMgtId", title: gfn_getMsg("pop_badInfo"), width: "120px",
			template:'<button class="k-button k-primary" type="button" onclick = "lfn_qualBadItemPop_Open(this) " style="width:60px;"><spring:message code = "pop_itemBadRegist"></spring:message></button>'
		}
	];
...
this.lfn_qualBadItemPop_Open = function(target){
	var options = {
		...
		id : "jteBadItemPop", 
		title:gfn_getMsg("pop_badSearch"),
		resizable : true,
...
*/
	...
	console.log(lfo_common.gridSelected);
	//5.
	...
	lfo_badItemGrd.columns = [
		...
		{field: "lotId", title : gfn_getMsg("col_itemIndivCd"), width: "180px"},
...
this.lfn_badItemGrd_open = function(mode, dataItem){
	...
	if(mode =="NEW"){
		options.title = gfn_getMsg("pop_itemBadRegist")
	}else if(mode=="EDT"){
		options.title = gfn_getMsg("pop_badItemSrh")
	}
