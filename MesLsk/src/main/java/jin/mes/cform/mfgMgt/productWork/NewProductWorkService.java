...
import jin.mes.cform.basMgt.operMgt.facilMgt.NewFacilMgtMapper;
import jin.mes.cform.basMgt.operMgt.rlehoMgt.NewRlehoMgtDto;
import jin.mes.cform.basMgt.operMgt.rlehoMgt.NewRlehoMgtMapper;
import jin.mes.cform.basMgt.operMgt.rlehoMgt.NewRlehoMgtService;
...
public class NewProductWorkService {
	...
	@Resource
	NewFacilMgtMapper newEqmtMgtMapper;
	
	@Resource
	NewFacilCorMgtMapper newEqmtCorMgtMapper;
	...
	@Resource
	NewRlehoMgtMapper placeMgtMapper;
	...
	public String setProdWorkAsgnSave(NewWrkinTeamMgtDto newAsgnDto) {
		...
		try {
			...
			if (newAsgnDto.getAction().equals("C")) {// 작업지시 등록
				...
				//설비라인에 설비목록 가져오기
				NewFacilMgtDto eqmtMgtDto = new NewFacilMgtDto();
				eqmtMgtDto.setPlaceId(newAsgnDto.getPlaceId());
				List<NewFacilMgtDto> eqmtMgtList = newEqmtMgtMapper.selectEqmtMgtListAll(eqmtMgtDto);
				
				//라우팅마다 기본 작업정보 생성
				for(int i =0; i< routingMgtList.size(); i++) {
					
					//작업정보 생성자
					NewWrkinTeamMgtDto newProdWorkRoutSheetDto = new NewWrkinTeamMgtDto(returnKey, null, newAsgnDto.getLotId(), null, 
							routingMgtList.get(i).getRoutingId(), eqmtMgtList.get(i).getEqmtMgtId(), null, newAsgnDto.getProdAsmQty(), null, newAsgnDto.getProdWorkState());
	...
	public String setProdWorkRoutSheetStart(NewWrkinTeamMgtDto newAsgnDto) {
		...
		try {
			...
			String nowDate = dateFormat.format(dateTime);
			
			newAsgnDto.setCallAsmModDt(nowDate);
			
			newProductWorkMapper.updateProdWorkAsgn(newAsgnDto);
			
			NewWrkinTeamMgtDto sheetDto = new NewWrkinTeamMgtDto();
			sheetDto.setProdAsmId(newAsgnDto.getProdAsmId());
			List<NewWrkinTeamMgtDto> workList = newProductWorkMapper.selectProdWorkRoutSheetList(sheetDto);
			
			for(NewWrkinTeamMgtDto workDto : workList) {
				workDto.setProdWorkStart(newAsgnDto.getProdWorkStart());
				workDto.setProdWorkState(newAsgnDto.getProdAsmState());
				workDto.setProdWorkUser(JteUtils.getUserId());
				
				newProductWorkMapper.updateProdWorkRoutSheet(workDto);
				returnKey = workDto.getProdAsmId();
				
				//설비 가동 시작 -- PLC로 체크하는경우 설비가동 삭제
				if(!StringUtils.isEmpty(workDto.getEqmtMgtId()) && !workDto.getEqmtMgtId().equals("")) {
					NewFacilCorMgtDto newEqmtMgtDto = new NewFacilCorMgtDto();
					newEqmtMgtDto.setEqmtMgtId(workDto.getEqmtMgtId());
					newEqmtMgtDto.setEqmtWorkStart(nowDate);
					newEqmtMgtDto.setProdWorkId(workDto.getProdWorkId());
					newEqmtCorMgtMapper.insertEqmtWork(newEqmtMgtDto);
				}
			}
		} catch (Exception e) {
			returnKey = "";
	...
	public String setProdWorkRoutSheetEnd(NewWrkinTeamMgtDto newAsgnDto) {
		...
		try {
			...
			//상태변경 최종 시간
			newAsgnDto.setCallAsmModDt(dateFormat.format(dateTime));
			...
			for(NewWrkinTeamMgtDto workDto : workList) {
				workDto.setProdWorkEnd(newAsgnDto.getProdWorkEnd());
				workDto.setProdWorkState(newAsgnDto.getProdAsmState());
				workDto.setProdWorkUser(JteUtils.getUserId());
				//workDto.setProdWorkGood((int)newAsgnDto.getProdAsmGood());
				
				newProductWorkMapper.updateProdWorkRoutSheet(workDto);
				returnKey = workDto.getProdWorkId();
				...
				if(!StringUtils.isEmpty(workDto.getEqmtMgtId()) && !workDto.getEqmtMgtId().equals("")) {
					...
					newEqmtMgtDto.setProdWorkId(workDto.getProdWorkId());
					newEqmtCorMgtMapper.updateEqmtWork(newEqmtMgtDto);
				}
			}
...
//	}	
	...
	public String setProdWorkItemBadSave(NewInfergodsMgtDto newItemBadDto){
		...
		try {
			...
			NewItemStockMgtDto newItemStockMgtDto = new NewItemStockMgtDto(newItemBadDto.getItemId(),newItemBadDto.getLotId(),newItemBadDto.getBadQty()*-1,
					"prod_tagt02", returnKey, null, newItemBadDto.getChkUser());
			newItemStockMgtDto.setAction(newItemBadDto.getAction());
			newItemStockService.setItemStockPgSave(newItemStockMgtDto);
			...
			if(newItemBadDto.getBadTarget().equals("qual_spec03")) {
				...
				NewWrkinTeamMgtDto newAsgnInfoDto = new NewWrkinTeamMgtDto();
				newAsgnInfoDto.setProdAsmId(newItemBadDto.getBadTargetCode());
				newAsgnInfoDto.setProdAsmBad(newProductWorkMapper.selectItemBadTotal(newItemBadTotalDto));
				newAsgnInfoDto.setCallAsmGood(newProductWorkMapper.selectItemCallBadTotal(newAsgnInfoDto));
				newAsgnInfoDto.setAction("U");
				
				newProductWorkMapper.updateProdWorkAsgn(newAsgnInfoDto);
			}
		} catch (Exception e) {
			returnKey = "";
