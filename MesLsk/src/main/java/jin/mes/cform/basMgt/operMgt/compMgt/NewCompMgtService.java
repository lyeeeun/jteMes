...
public class NewCompMgtService {
	...
	//업체 상세주소 조회
	public PageInfo<NewCompAddrDto> getCompAddrList(NewCompAddrDto newcompAddrDto, PageRequestVo pageRequestVo){
		List<NewCompAddrDto> compAddrList = null;
		int rowCount = 0;

		// Page Setting
		newcompAddrDto.setFirstIndex(pageRequestVo.getBlockFirstPage());
		newcompAddrDto.setLastIndex(pageRequestVo.getBlockLastPage());

		try {
			//Count
			rowCount = newCompMgtMapper.selectCompAddrCount(newcompAddrDto);
			if (rowCount > 0) {
				//List
				compAddrList = newCompMgtMapper.selectCompAddrList(newcompAddrDto);
			}
		} catch (Exception e) {
	...
	//업체 상세주소 저장
	@Transactional
	public String setCompAddrSave(NewCompAddrDto newcompAddrDto){
		
		String resultKey = "";
		try {
			newcompAddrDto.setCreatorId(JteUtils.getUserId());
			newcompAddrDto.setUpdatorId(JteUtils.getUserId());
			
			if(newcompAddrDto.getAction().equals("C")) {//
				//업체입력
				newCompMgtMapper.insertCompAddrInfo(newcompAddrDto);
				resultKey = newcompAddrDto.getCompAddrId();
				
				//다국어 저장
				if(newcompAddrDto.getMsgList() != null && newcompAddrDto.getMsgList().size() > 0) {
					msgService.pgSaveMsg(newcompAddrDto.getMsgList(), "compadr", resultKey);
				}
			}else if(newcompAddrDto.getAction().equals("U")){//업체 변경
				//업체수정
				newCompMgtMapper.updateCompAddrInfo(newcompAddrDto);
				resultKey = newcompAddrDto.getCompAddrId();
				//다국어 저장
				if(newcompAddrDto.getMsgList() != null && newcompAddrDto.getMsgList().size() > 0) {
					msgService.pgSaveMsg(newcompAddrDto.getMsgList(), "compadr", resultKey);
				}
			}else if(newcompAddrDto.getAction().equals("USE")){//업체 잠금/해제
				if(newcompAddrDto.isUse()) {
					newcompAddrDto.setUse(false);
				}else {
					newcompAddrDto.setUse(true);
				}
				//업체사용수정	
				newCompMgtMapper.updateCompAddrInfo(newcompAddrDto);
				resultKey = newcompAddrDto.getCompAddrId();
	...
	public int setCompAddrDelete(List<NewCompAddrDto> compAddrList){
		...
		try {
			//업체삭제
			for(NewCompAddrDto newcompAddrDto : compAddrList){
				newCompMgtMapper.deleteCompAddrInfo(newcompAddrDto);
			}
		} catch (Exception e) {
			resultInt = -1;
