...
public class NewUserInfoController extends BaseController {
	...
	public ModelMap getUserList(ModelMap model, @ModelAttribute("newUserInfo") NewUserInfoDto newUserInfo) {
		PageRequestVo pageRequestVo = new PageRequestVo();
		pageRequestVo.setPage(getCurrentPage(Integer.toString(newUserInfo.getPage())));
		pageRequestVo.setRows(newUserInfo.getRows());

		try {
			setModelWithkendoList(model, newUserInfoService.getUserList(newUserInfo, pageRequestVo));
		} catch (Exception ex) {
	...
	public ResponseEntity<Object> getUserRoleList(ModelMap model, @ModelAttribute("newUserAuthDto") NewUserAuthDto newUserAuthDto) {
		return new ResponseEntity<Object>(newUserInfoService.getUserRoleList(newUserAuthDto), HttpStatus.OK);
	}
...
//			final JPQLQuery<Account> query;
	...
	@ResponseBody
	public int setUserSave(ModelMap model, @RequestBody NewUserInfoDto newUserInfo) {
		int reutrnInt = 0;
		try {
			reutrnInt = newUserInfoService.setUserSave(newUserInfo);
		} catch (Exception e) {
