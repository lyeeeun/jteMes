...
this.gfn_pwChangePop = function(){
	var options = {
		modal:true, 
		width: "420px", 
		height: "180px", /*원래 350, 230*/
		id:"jtePwChangeForm", 
		title:"비밀번호 변경(6개월 초과)", 
		workCd:"", 
		bdSeq:"",
		resizable:true,
		animation:{open :{effects:"expand:vertical fadeIn"},
...
this.gfn_myConfigSave = function(formID){
	...
	if($("#t_passwordYN").is(":checked") || formID == "jtePwChangeForm_form"){
		...
		if(!pwdPtn.test(formObj.password)){
			gfn_msgBox({msg : gfn_getMsg("pop_pwLetterChar")}); //패스워드는8~16글자 특수문자 1글자 이상 포함되어야 합니다.
			...
			gfn_msgBox({msg : gfn_getMsg("pop_pwMismatch")}); //패스워드가 일치하지 않습니다.
	...
	$.ajax({
		async:false,
		url : "/cform/basMgt/userMgt/userInfo/setUserSave",
		type: "POST",
...
this.gfn_ajaxCallByList = function(type, prmt){
	...
	switch(type){
		case "place" :	//위치정보
			typeObj.url = "/cform/basMgt/operMgt/rlehoMgt/getPlaceListAll";
		break;
		case "user" : //사용자정보
			typeObj.url = "/cform/basMgt/userMgt/userInfo/getUserList";
		break
		case "svcMenu" : //사용자정보
