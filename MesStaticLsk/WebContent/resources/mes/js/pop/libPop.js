...
*/		];
	...
	case "tool"://공구 마스터 정보
		comPopObj.type = type;
		comPopObj.callback = callback;
		comPopObj.width = 840; /* 201022 JJW 670- > 840 */
		comPopObj.height = 320; /* 201022 JJW 315-> 320*/
...
*/			{ text: gfn_getMsg("col_desc"), value: "toolInfo.tool_desc" }			//설명
	...
	case "toolMgt":
		...
		comPopObj.height = 315;
		comPopObj.title = gfn_getMsg("pop_toolInfo");			//공구정보
		comPopObj.url = "/cform/mtrl/mtrltoolMgt/getToolMgtList";
		comPopObj.prmtNm = ["use"];
		comPopObj.prmtVal = [true];
		comPopObj.model = {
				id: "toolMgtId",
		...
		comPopObj.columns =[
			{field: "toolMgtId", title:gfn_getMsg("col_toolCd"), width: 150},									//공구코드
			{field: "toolNm", title:gfn_getMsg("col_toolNm"), width: 150},									//공구명
			{field: "toolMgtDesc", title:gfn_getMsg("col_desc"), width: 250}								//설명
		];
		comPopObj.shBoxItem = [
			{ text: gfn_getMsg("col_toolCd"), value: "toolMgt.tool_mgt_id" },			//공구코드
			{ text: gfn_getMsg("col_toolNm"), value: "toolInfo.tool_nm" }			//공구명
		];
		break;	
	case "item"://부품정보
		...
		comPopObj.columns =[
			...
			{field: "placeNm", title : "생산라인", width: "100px"},
			{field: "itemStdStr02", title : "위치코드", width: "100px"},
...
*/			{field: "itemStdStr01", title : gfn_getMsg("col_partSpecification"), width: "100px"},			//부품규격
...
*/			{field: "mtrlStd03", format: "{0:n}", title:gfn_getMsg("col_weight"), width: "110px"},				//무게
			...
			{field: "mtrlStdStr01", title:gfn_getMsg("col_mtrlStandard"), width: "260px"},
	...
	case "materialMgt"://자재상세
		...
		comPopObj.columns =[
			...
			{field: "mtrlStdStr01", title:gfn_getMsg("col_mtrlStandard"), width: 200},
...
*/			{field: "itemNm", title:gfn_getMsg("col_itemNm"), width: "220px;"},				//부품명
	...
	case "assignInfo": //Lot 별 작업 지시 정보
		...
		comPopObj.prmtNm = [];
		comPopObj.prmtVal = [];
		comPopObj.url = "/cform/mfgMgt/productWork/getProdWorkAsgnList";
		comPopObj.model = {
				fields: {
...
//		];
	...
	case "place"://위치정보
		...
		comPopObj.width = 670;
		comPopObj.height = 315;
		comPopObj.title = gfn_getMsg("pop_placeInfo");			//자재상세
		comPopObj.url = "/cform/basMgt/operMgt/rlehoMgt/getPlaceList";
		comPopObj.prmtNm = ["use"];
		comPopObj.prmtVal = [true];
		comPopObj.model = {
			id: "placeId",
		...
		comPopObj.columns =[
			...
			{field: "placeSeq", title:"순서", width: "80px"},
...
// 검사(입고, 자주&공정, 출고) 팝업 
this.gfn_inspectPop = function(mode, inspectMode, popId, param) {
	...
	if(inspectMode == "selfInspectDct") {
		option.width = 760; //201005 700->760
		option.height = 530; //201005 500->535
		option.title = "DCT 자주 검사";			//자재정보
		option.baseGroupCode = "qual_self_insp_dct";
		option.category = "self_inspect_dct";
	} else if(inspectMode == "selfInspectAnn") {
		option.width = 700;
		option.height = 500;
		option.title = "ANN 자주 검사";			//자재정보
		option.baseGroupCode = "qual_self_insp_ann";
		option.category = "self_inspect_ann";
	} else if(inspectMode == "incommingInspect") {
		option.width = 760;
		option.height = 530;
		option.title = "입고 검사";			//발주현황 (자재정보), 입고검사
		option.baseGroupCode = "qual_incomming_insp";
		option.category = "incomming_inspect";
	} else if(inspectMode == "shipmentInspect") {
		option.width = 760;
		option.height = 530;
		option.title = "출고 검사";			//포장진행현황 (자재정보), 출고검사
		option.baseGroupCode = "qual_shipment_insp";
		option.category = "shipment_inspect";
	} else {
	...
	$("#" + option.popAreaId).append("<table id='" + option.popAreaTableId + "' style='width:100%;' class='mtrlOdcursitMgtForm-table'>" +
			...
			"<td>" + gfn_getMsg("col_inspectionPort") + "</td>" +
			"<td>" + gfn_getMsg("col_mtrlOdcursitCycle") + "</td>" +
	...
	$("#" + option.popAreaId).append("<div class='mtrlOdcursitMgtForm final-btn-area' style='text-align:center;'>" +
			"<button class='k-button k-primary final-btn' type='button' onclick='gfn_inspectPop_save(\"" + popId + "\", \"" + param + "\", \"" + option.category + "\", \"" + mode + "\")'>" + gfn_getMsg("pop_save") + "</spring:message></button>" +
	...
	if(mode == "EDT") {
		$.ajax({
			async:false,
			url : "/cform/qualMgt/qualCheckList/selectCheckList",
			type: "POST",
			data: {inspectId: param},
			traditional :true,
...
this.gfn_inspectPop_save = function(popId, param, category, mode){
	var insertInspectList = new Array();
	var updateInspectList = new Array();
	var svcUrl = "/cform/qualMgt/qualCheckList";
	var savePrmt = {};
	...
	if(mode == "NEW") {
		$("#" + popId + "_Form_tb").find('tr').each(function(index, item) {
			...
			insertInspectList.push({ questionId: $(item).attr('id'), questionValue: $(item).find('input').val(), targetId: param, category: category});
		});
		...
		$("#" + popId + "_Form_tb").find('tr').each(function(index, item) {
			...
			updateInspectList.push({ questionId: $(item).attr('id'), questionValue: $(item).find('input').val(), inspectId: param});
		});
